...
extends FileDescriptor {
    ...
    public final int sendTo(ByteBuffer buf, int pos, int limit, InetAddress addr, int port) throws IOException {
        ...
        int res = Socket.sendTo(this.fd, this.ipv6, buf, pos, limit, address, scopeId, port);
        if (res >= 0) {
            return res;
        }
    ...
    public final int sendToAddress(long memoryAddress, int pos, int limit, InetAddress addr, int port) throws IOException {
        ...
        int res = Socket.sendToAddress(this.fd, this.ipv6, memoryAddress, pos, limit, address, scopeId, port);
        if (res >= 0) {
            return res;
        }
        if (res == Errors.ERROR_ECONNREFUSED_NEGATIVE) {
    ...
    public final int sendToAddresses(long memoryAddress, int length, InetAddress addr, int port) throws IOException {
        ...
        int res = Socket.sendToAddresses(this.fd, this.ipv6, memoryAddress, length, address, scopeId, port);
        if (res >= 0) {
            return res;
        }
        if (res == Errors.ERROR_ECONNREFUSED_NEGATIVE) {
    ...
    public final boolean connect(SocketAddress socketAddress) throws IOException {
        ...
        if (socketAddress instanceof InetSocketAddress) {
            ...
            res = Socket.connect(this.fd, this.ipv6, address.address, address.scopeId, inetSocketAddress.getPort());
    ...
    public final void bind(SocketAddress socketAddress) throws IOException {
        if (socketAddress instanceof InetSocketAddress) {
            ...
            int res = Socket.bind(this.fd, this.ipv6, address.address, address.scopeId, addr.getPort());
            if (res < 0) {
