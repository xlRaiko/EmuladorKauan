...
extends ByteBuf {
    ...
    final void ensureWritable0(int minWritableBytes) {
        ...
        if (checkBounds && targetCapacity > this.maxCapacity) {
            this.ensureAccessible();
    ...
    public int indexOf(int fromIndex, int toIndex, byte value) {
        if (fromIndex <= toIndex) {
            return this.firstIndexOf(fromIndex, toIndex, value);
        }
        return this.lastIndexOf(fromIndex, toIndex, value);
    }

    private int firstIndexOf(int fromIndex, int toIndex, byte value) {
        if ((fromIndex = Math.max(fromIndex, 0)) >= toIndex || this.capacity() == 0) {
            return -1;
        }
        this.checkIndex(fromIndex, toIndex - fromIndex);
        for (int i = fromIndex; i < toIndex; ++i) {
            if (this._getByte(i) != value) continue;
            return i;
        }
        return -1;
    }
    ...
    private int lastIndexOf(int fromIndex, int toIndex, byte value) {
        if ((fromIndex = Math.min(fromIndex, this.capacity())) < 0 || this.capacity() == 0) {
            return -1;
        }
        this.checkIndex(toIndex, fromIndex - toIndex);
