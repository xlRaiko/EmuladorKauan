...
IEventTriggers {
    ...
    private static Class[] TOGGLING_INTERACTIONS = new Class[]{InteractionGameTimer.class, InteractionWired.class, InteractionWiredHighscore.class, InteractionMultiHeight.class};
    private int id;
    private int userId;
    private int roomId;
    ...
    public void serializeFloorData(ServerMessage serverMessage) {
        try {
            ...
            serverMessage.appendString((String)(this.getBaseItem().getInteractionType().getType() == InteractionTrophy.class || this.getBaseItem().getInteractionType().getType() == InteractionCrackable.class || this.getBaseItem().getName().equalsIgnoreCase("gnome_box") ? "1.0" : (this.getBaseItem().allowWalk() || this.getBaseItem().allowSit() && this.roomId != 0 ? "" + Item.getCurrentHeight(this) : "")));
        }
        catch (Exception e) {
    ...
    public void run() {
        block23: {
            try (Connection connection = Emulator.getDatabase().getDataSource().getConnection();){
                ...
                if (!this.needsUpdate) break block23;
                try (PreparedStatement statement = connection.prepareStatement("UPDATE items SET user_id = ?, room_id = ?, wall_pos = ?, x = ?, y = ?, z = ?, rot = ?, extra_data = ?, limited_data = ? WHERE id = ?");){
                    ...
                    statement.setDouble(6, Math.max(-9999.0, Math.min(9999.0, (double)Math.round(this.z * Math.pow(10.0, 6.0)) / Math.pow(10.0, 6.0))));
                    ...
                    LOGGER.error("SQLException trying to save HabboItem: {}", (Object)this.toString());
                }
                this.needsUpdate = false;
            }
            catch (SQLException e) {
    ...
    public void onClick(GameClient client, Room room, Object[] objects) throws Exception {
        if (client != null && this.getBaseItem().getType() == FurnitureType.FLOOR) {
            ...
            if (this.getBaseItem().getStateCount() > 1 && !(this instanceof InteractionDice) || Arrays.asList(TOGGLING_INTERACTIONS).contains(this.getClass()) || objects != null && objects.length == 1 && objects[0].equals("TOGGLE_OVERRIDE")) {
                WiredHandler.handle(WiredTriggerType.STATE_CHANGED, client.getHabbo().getRoomUnit(), room, new Object[]{this});
                WiredHandler.handle(WiredTriggerType.STATE_FURNI, client.getHabbo().getRoomUnit(), room, new Object[]{this});
            }
        }
    }
    ...
    public void onWalkOn(RoomUnit roomUnit, Room room, Object[] objects) throws Exception {
        ...
        if ((this.getBaseItem().allowSit() || this.getBaseItem().allowLay()) && !roomUnit.getDanceType().equals((Object)DanceType.NONE)) {
            roomUnit.setDanceType(DanceType.NONE);
            room.sendComposer(new RoomUserDanceComposer(roomUnit).compose());
        }
        if (!this.getBaseItem().getClothingOnWalk().isEmpty() && roomUnit.getPreviousLocation() != roomUnit.getGoal() && roomUnit.getGoal() == room.getLayout().getTile(this.x, this.y) && (habbo = room.getHabbo(roomUnit)) != null && habbo.getClient() != null) {
            String[] clothingKeys = (String[])Arrays.stream(this.getBaseItem().getClothingOnWalk().split("\\.")).map(k -> k.split("-")[0]).toArray(String[]::new);
            habbo.getHabboInfo().setLook(String.join((CharSequence)".", (CharSequence[])Arrays.stream(habbo.getHabboInfo().getLook().split("\\.")).filter(k -> !ArrayUtils.contains(clothingKeys, k.split("-")[0])).toArray(String[]::new)) + "." + this.getBaseItem().getClothingOnWalk());
    ...
    public void onPlace(Room room) {
        ...
        int furniCollecterProgress = owner == null ? AchievementManager.getAchievementProgressForHabbo(this.getUserId(), roomDecoAchievement) : owner.getHabboStats().getAchievementProgress(roomDecoAchievement);
        ...
        int uniqueFurniCollecterProgress = owner == null ? AchievementManager.getAchievementProgressForHabbo(this.getUserId(), roomDecoUniqueAchievement) : owner.getHabboStats().getAchievementProgress(roomDecoUniqueAchievement);
    ...
    public void onPickUp(Room room) {
        if (this.getBaseItem().getEffectF() > 0 || this.getBaseItem().getEffectM() > 0) {
            ...
            for (Habbo habbo : room.getHabbosOnItem(this)) {
                if (this.getBaseItem().getEffectM() > 0 && habbo.getHabboInfo().getGender().equals((Object)HabboGender.M) && habbo.getRoomUnit().getEffectId() == this.getBaseItem().getEffectM()) {
                    room.giveEffect(habbo, nextEffectM, -1);
                }
                if (this.getBaseItem().getEffectF() <= 0 || !habbo.getHabboInfo().getGender().equals((Object)HabboGender.F) || habbo.getRoomUnit().getEffectId() != this.getBaseItem().getEffectF()) continue;
                room.giveEffect(habbo, nextEffectF, -1);
            }
            for (Bot bot : room.getBotsAt(room.getLayout().getTile(this.getX(), this.getY()))) {
                if (this.getBaseItem().getEffectM() > 0 && bot.getGender().equals((Object)HabboGender.M) && bot.getRoomUnit().getEffectId() == this.getBaseItem().getEffectM()) {
                    room.giveEffect(bot.getRoomUnit(), nextEffectM, -1);
                }
                if (this.getBaseItem().getEffectF() <= 0 || !bot.getGender().equals((Object)HabboGender.F) || bot.getRoomUnit().getEffectId() != this.getBaseItem().getEffectF()) continue;
    ...
    public void onMove(Room room, RoomTile oldLocation, RoomTile newLocation) {
        if (this.getBaseItem().getEffectF() > 0 || this.getBaseItem().getEffectM() > 0) {
            ...
            ArrayList<Habbo> oldHabbos = new ArrayList<Habbo>();
            ArrayList<Habbo> newHabbos = new ArrayList<Habbo>();
            ArrayList<Bot> oldBots = new ArrayList<Bot>();
            ArrayList<Bot> newBots = new ArrayList<Bot>();
    ...
    public List<RoomTile> getOccupyingTiles(RoomLayout layout) {
        ArrayList<RoomTile> tiles = new ArrayList<RoomTile>();
        ...
        int i = rect.x;
        while ((double)i < (double)rect.x + rect.getWidth()) {
            int j = rect.y;
            while ((double)j < (double)rect.y + rect.getHeight()) {
    ...
    public List<RoomTile> getOccupingTilesAround(RoomLayout layout) {
        ArrayList<RoomTile> tiles = new ArrayList<RoomTile>();
        ...
        while ((double)i < (double)rect.x + rect.getWidth()) {
            ...
            while ((double)j < (double)rect.y + rect.getHeight()) {
                for (RoomUserRotation rot : rotations) {
                    RoomTile tile = layout.getTile((short)i, (short)j);
                    tiles.add(layout.getTileInFront(tile, rot.getValue()));
                }
                ++j;
            }
            ++i;
    ...
    public Rectangle getRectangle(int marginX, int marginY) {
        return RoomLayout.getRectangle(this.getX() - marginX, this.getY() - marginY, this.getBaseItem().getWidth() + marginX * 2, this.getBaseItem().getLength() + marginY * 2, this.getRotation());
    }
}
