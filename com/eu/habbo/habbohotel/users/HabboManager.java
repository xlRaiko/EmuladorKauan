...
public class HabboManager {
    ...
    public HabboManager() {
        ...
        LOGGER.info("Habbo Manager -> Loaded! ({} MS)", (Object)(System.currentTimeMillis() - millis));
    }
    ...
    public Habbo getHabbo(String username) {
        ConcurrentHashMap<Integer, Habbo> concurrentHashMap = this.onlineHabbos;
        synchronized (concurrentHashMap) {
            for (Map.Entry<Integer, Habbo> map : this.onlineHabbos.entrySet()) {
                if (!map.getValue().getHabboInfo().getUsername().equalsIgnoreCase(username)) continue;
                return map.getValue();
            }
        }
        return null;
    ...
    public Habbo loadHabbo(String sso) {
        ...
        block55: {
            ...
            if ((ban = Emulator.getGameEnvironment().getModToolManager().checkForBan(userId)) != null) {
                return null;
            }
            try (Connection connection = Emulator.getDatabase().getDataSource().getConnection();
    ...
    public HabboInfo getHabboInfo(int id) {
        if (this.getHabbo(id) == null) {
            return HabboManager.getOfflineHabboInfo(id);
        }
    ...
    public void sendPacketToHabbosWithPermission(ServerMessage message, String perm) {
        ...
        synchronized (concurrentHashMap) {
            for (Habbo habbo : this.onlineHabbos.values()) {
                if (!habbo.hasPermission(perm)) continue;
    ...
    public ArrayList<HabboInfo> getCloneAccounts(Habbo habbo, int limit) {
        ...
        try (Connection connection = Emulator.getDatabase().getDataSource().getConnection();
             PreparedStatement statement = connection.prepareStatement("SELECT * FROM users WHERE ip_register = ? OR ip_current = ? AND id != ? ORDER BY id DESC LIMIT ?");){
    ...
    public List<Map.Entry<Integer, String>> getNameChanges(int userId, int limit) {
        ArrayList<Map.Entry<Integer, String>> nameChanges = new ArrayList<Map.Entry<Integer, String>>();
        try (Connection connection = Emulator.getDatabase().getDataSource().getConnection();
             PreparedStatement statement = connection.prepareStatement("SELECT timestamp, new_name FROM namechange_log WHERE user_id = ? ORDER by timestamp DESC LIMIT ?");){
    ...
    public void staffAlert(String message) {
        ...
        ServerMessage msg = new GenericAlertComposer((String)message).compose();
