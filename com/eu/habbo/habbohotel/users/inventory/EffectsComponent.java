...
public class EffectsComponent {
    ...
    public HabboEffect createEffect(int effectId, int duration) {
        HabboEffect effect;
        THashMap<Integer, HabboEffect> tHashMap = this.effects;
    ...
    public HabboEffect createRankEffect(int effectId) {
        HabboEffect rankEffect = new HabboEffect(effectId, this.habbo.getHabboInfo().getId());
    ...
    public void dispose() {
        ...
        synchronized (tHashMap) {
            try (Connection connection = Emulator.getDatabase().getDataSource().getConnection();
                 PreparedStatement statement = connection.prepareStatement("UPDATE users_effects SET duration = ?, activation_timestamp = ?, total = ? WHERE user_id = ? AND effect = ?");){
