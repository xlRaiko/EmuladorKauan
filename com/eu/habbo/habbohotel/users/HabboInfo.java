...
implements Runnable {
    ...
    private List<NavigatorSavedSearch> savedSearches = new ArrayList<NavigatorSavedSearch>();
    private List<MessengerCategory> messengerCategories = new ArrayList<MessengerCategory>();
    ...
    public HabboInfo(ResultSet set) {
        try {
            ...
            if (this.rank == null) {
                LOGGER.error("No existing rank found with id {}. Make sure an entry in the permissions table exists.", (Object)set.getInt("rank"));
                LOGGER.warn("{} has an invalid rank with id {}. Make sure an entry in the permissions table exists.", (Object)this.username, (Object)set.getInt("rank"));
    ...
    private void saveCurrencies() {
        try (Connection connection = Emulator.getDatabase().getDataSource().getConnection();
             final PreparedStatement statement = connection.prepareStatement("INSERT INTO users_currency (user_id, type, amount) VALUES (?, ?, ?) ON DUPLICATE KEY UPDATE amount = ?");){
            this.currencies.forEachEntry(new TIntIntProcedure(){
                final /* synthetic */ HabboInfo this$0;
                {
                    this.this$0 = this$0;
                }
                ...
                public boolean execute(int a, int b) {
                    try {
                        statement.setInt(1, this.this$0.getId());
    ...
    private void loadSavedSearches() {
        this.savedSearches = new ArrayList<NavigatorSavedSearch>();
        try (Connection connection = Emulator.getDatabase().getDataSource().getConnection();
    ...
    private void loadMessengerCategories() {
        this.messengerCategories = new ArrayList<MessengerCategory>();
        try (Connection connection = Emulator.getDatabase().getDataSource().getConnection();
    ...
    public void dismountPet(boolean isRemoving) {
        ...
        if ((roomUnit = habbo.getRoomUnit()) == null) {
            return;
        }
        ...
        ArrayList availableTiles = isRemoving ? new ArrayList() : this.getCurrentRoom().getLayout().getWalkableTilesAround(roomUnit.getCurrentLocation());
        RoomTile tile = availableTiles.isEmpty() ? roomUnit.getCurrentLocation() : (RoomTile)availableTiles.get(0);
    ...
    public void run() {
        this.saveCurrencies();
        try (Connection connection = Emulator.getDatabase().getDataSource().getConnection();
             PreparedStatement statement = connection.prepareStatement("UPDATE users SET motto = ?, online = ?, look = ?, gender = ?, credits = ?, last_login = ?, last_online = ?, home_room = ?, ip_current = ?, `rank` = ?, machine_id = ?, username = ? WHERE id = ?");){
