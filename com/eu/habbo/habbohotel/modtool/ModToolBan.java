...
implements Runnable {
    ...
    public void run() {
        if (this.needsInsert) {
            try (Connection connection = Emulator.getDatabase().getDataSource().getConnection();
                 PreparedStatement statement = connection.prepareStatement("INSERT INTO bans (user_id, ip, machine_id, user_staff_id, timestamp, ban_expire, ban_reason, type, cfh_topic) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");){
    ...
    public String listInfo() {
        return "Banned User Id: " + this.userId + "\rType: " + this.type.getType() + "\rReason: <i>" + this.reason + "</i>\rModerator Id: " + this.staffId + "\rDate: " + dateFormat.format((long)this.timestamp * 1000L) + "\rExpire Date: " + dateFormat.format((long)this.expireDate * 1000L) + "\rIP: " + this.ip + "\rMachineID: " + this.machineId + "\rTopic: " + this.cfhTopic;
    }
}
