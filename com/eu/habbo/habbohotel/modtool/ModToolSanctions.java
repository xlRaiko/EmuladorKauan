...
public class ModToolSanctions {
    ...
    public ModToolSanctions() {
        ...
        LOGGER.info("Sanctions Manager -> Loaded! ({} MS)", (Object)(System.currentTimeMillis() - millis));
    }
    ...
    public THashMap<Integer, ArrayList<ModToolSanctionItem>> getSanctions(int habboId) {
        THashMap<Integer, ArrayList<ModToolSanctionItem>> tHashMap = this.sanctionHashmap;
            ...
            try (Connection connection = Emulator.getDatabase().getDataSource().getConnection();
                ...
                try (ResultSet set = statement.executeQuery();){
                    while (set.next()) {
                        ...
                        if (this.sanctionHashmap.get(set.getInt("habbo_id")).contains(item)) continue;
    ...
    private void insertSanction(int habboId, int sanctionLevel, int probationTimestamp, String reason, int tradeLockedUntil, boolean isMuted, int muteDuration) {
        try (Connection connection = Emulator.getDatabase().getDataSource().getConnection();
             PreparedStatement statement = connection.prepareStatement("INSERT INTO sanctions (habbo_id, sanction_level, probation_timestamp, reason, trade_locked_until, is_muted, mute_duration) VALUES (?, ?, ?, ?, ?, ?, ?)");){
            statement.setInt(1, habboId);
    ...
    private void runSanctionBasedOnLevel(ModToolSanctionLevelItem sanctionLevelItem, int habboId, String reason, int cfhTopic, Habbo self, int muteDuration) {
        ...
        if (muteDuration > 0) {
            Date muteDurationDate = new Date((long)muteDuration * 1000L);
        ...
        switch (sanctionLevelItem.sanctionType) {
            ...
            case "BAN": {
                Emulator.getGameEnvironment().getModToolManager().ban(habboId, self, reason, sanctionLevelItem.sanctionHourLength, ModToolBanType.ACCOUNT, cfhTopic);
                break;
            }
            case "MUTE": {
