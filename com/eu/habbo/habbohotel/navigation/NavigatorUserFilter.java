...
public class NavigatorUserFilter
extends NavigatorFilter {
    ...
    public List<SearchResultList> getResult(Habbo habbo) {
        ...
        List<Room> friendRooms;
        List<Room> groupRooms;
        List<Room> frequentlyVisited;
        int i = 0;
        ...
        if (!(frequentlyVisited = Emulator.getGameEnvironment().getNavigatorManager().getRoomsForCategory("history_freq", habbo)).isEmpty()) {
        ...
        if (!(groupRooms = Emulator.getGameEnvironment().getNavigatorManager().getRoomsForCategory("my_groups", habbo)).isEmpty()) {
        ...
        if (!(friendRooms = Emulator.getGameEnvironment().getNavigatorManager().getRoomsForCategory("with_friends", habbo)).isEmpty()) {
        ...
        if (!(rightRooms = Emulator.getGameEnvironment().getNavigatorManager().getRoomsForCategory("with_rights", habbo)).isEmpty()) {
