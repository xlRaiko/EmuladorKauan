...
public class Messenger {
    ...
    public static void unfriend(int userOne, int userTwo) {
        try (Connection connection = Emulator.getDatabase().getDataSource().getConnection();
             PreparedStatement statement = connection.prepareStatement("DELETE FROM messenger_friendships WHERE (user_one_id = ? AND user_two_id = ?) OR (user_one_id = ? AND user_two_id = ?)");){
    ...
    public static THashSet<MessengerBuddy> searchUsers(String username) {
        ...
        try (Connection connection = Emulator.getDatabase().getDataSource().getConnection();
             PreparedStatement statement = connection.prepareStatement("SELECT * FROM users WHERE username LIKE ? ORDER BY username ASC LIMIT " + Emulator.getConfig().getInt("hotel.messenger.search.maxresults"));){
    ...
    public static boolean canFriendRequest(Habbo habbo, String friend) {
        ...
        HabboInfo habboInfo = user != null ? user.getHabboInfo() : HabboManager.getOfflineHabboInfo(friend);
        if (habboInfo == null) {
            return false;
        }
        try (Connection connection = Emulator.getDatabase().getDataSource().getConnection();
             PreparedStatement statement = connection.prepareStatement("SELECT * FROM messenger_friendrequests WHERE (user_to_id = ? AND user_from_id = ?) OR (user_to_id = ? AND user_from_id = ?) LIMIT 1");){
    ...
    public static boolean friendRequested(int userFrom, int userTo) {
        try (Connection connection = Emulator.getDatabase().getDataSource().getConnection();
             PreparedStatement statement = connection.prepareStatement("SELECT * FROM messenger_friendrequests WHERE user_to_id = ? AND user_from_id = ? LIMIT 1");){
    ...
    public static THashMap<Integer, THashSet<MessengerBuddy>> getFriends(int userId) {
        ...
        try (Connection connection = Emulator.getDatabase().getDataSource().getConnection();
             PreparedStatement statement = connection.prepareStatement("SELECT users.id, users.look, users.username, messenger_friendships.relation FROM messenger_friendships INNER JOIN users ON users.id = messenger_friendships.user_two_id WHERE user_one_id = ? ORDER BY RAND() LIMIT 50");){
            statement.setInt(1, userId);
            try (ResultSet set = statement.executeQuery();){
                while (set.next()) {
                    ...
                    map.get(buddy.getRelation()).add(buddy);
                }
            }
        }
        catch (SQLException e) {
    ...
    public static void checkFriendSizeProgress(Habbo habbo) {
        ...
        if (progress > 0 && (toProgress = habbo.getMessenger().getFriends().size() - progress) < 0) {
            return;
        }
    ...
    public void loadFriends(Habbo habbo) {
        try (Connection connection = Emulator.getDatabase().getDataSource().getConnection();
             PreparedStatement statement = connection.prepareStatement("SELECT users.id, users.username, users.gender, users.online, users.look, users.motto, messenger_friendships.* FROM messenger_friendships INNER JOIN users ON messenger_friendships.user_two_id = users.id WHERE user_one_id = ?");){
            statement.setInt(1, habbo.getHabboInfo().getId());
            try (ResultSet set = statement.executeQuery();){
                while (set.next()) {
                    MessengerBuddy buddy = new MessengerBuddy(set);
                    if (buddy.getId() == habbo.getHabboInfo().getId()) continue;
    ...
    public MessengerBuddy loadFriend(Habbo habbo, int userId) {
        ...
        try (Connection connection = Emulator.getDatabase().getDataSource().getConnection();
             PreparedStatement statement = connection.prepareStatement("SELECT users.id, users.username, users.gender, users.online, users.look, users.motto, messenger_friendships.* FROM messenger_friendships INNER JOIN users ON messenger_friendships.user_two_id = users.id WHERE user_one_id = ? AND user_two_id = ? LIMIT 1");){
            statement.setInt(1, habbo.getHabboInfo().getId());
    ...
    public void loadFriendRequests(Habbo habbo) {
        try (Connection connection = Emulator.getDatabase().getDataSource().getConnection();
             PreparedStatement statement = connection.prepareStatement("SELECT users.id, users.username, users.look FROM messenger_friendrequests INNER JOIN users ON user_from_id = users.id WHERE user_to_id = ?");){
            statement.setInt(1, habbo.getHabboInfo().getId());
            try (ResultSet set = statement.executeQuery();){
                while (set.next()) {
    ...
    public THashSet<MessengerBuddy> getFriends(String username) {
        ...
        for (Map.Entry<Integer, MessengerBuddy> map : this.friends.entrySet()) {
            if (!StringUtils.containsIgnoreCase(map.getValue().getUsername(), username)) continue;
    ...
    public void connectionChanged(Habbo owner, boolean online, boolean inRoom) {
        if (owner != null) {
            for (Map.Entry<Integer, MessengerBuddy> map : this.getFriends().entrySet()) {
                ...
                Habbo habbo;
                if (map.getValue().getOnline() == 0 || (habbo = Emulator.getGameServer().getGameClientManager().getHabbo(map.getKey())) == null || habbo.getMessenger() == null || (buddy = habbo.getMessenger().getFriend(owner.getHabboInfo().getId())) == null) continue;
                ...
                habbo.getClient().sendResponse(new UpdateFriendComposer(habbo, buddy, (Integer)0));
            }
        }
    }
    ...
    public int deleteFriendRequests(int userFrom, int userTo) {
        try (Connection connection = Emulator.getDatabase().getDataSource().getConnection();){
            int n;
            block14: {
                PreparedStatement statement = connection.prepareStatement("DELETE FROM messenger_friendrequests WHERE (user_to_id = ? AND user_from_id = ?) OR (user_to_id = ? AND user_from_id = ?)");
                try {
                    statement.setInt(1, userTo);
                    statement.setInt(2, userFrom);
    ...
    public void acceptFriendRequest(int userFrom, int userTo) {
        ...
        if (count > 0) {
            try (Connection connection = Emulator.getDatabase().getDataSource().getConnection();
                 PreparedStatement statement = connection.prepareStatement("INSERT INTO messenger_friendships (user_one_id, user_two_id, friends_since) VALUES (?, ?, ?)");){
                statement.setInt(1, userFrom);
                statement.setInt(2, userTo);
            ...
            if (habboTo != null && habboFrom != null) {
                ...
                habboTo.getClient().sendResponse(new UpdateFriendComposer(habboTo, to, (Integer)1));
                habboFrom.getClient().sendResponse(new UpdateFriendComposer(habboFrom, from, (Integer)1));
            } else if (habboTo != null) {
                habboTo.getClient().sendResponse(new UpdateFriendComposer(habboTo, this.loadFriend(habboTo, userFrom), (Integer)1));
            } else if (habboFrom != null) {
                habboFrom.getClient().sendResponse(new UpdateFriendComposer(habboFrom, this.loadFriend(habboFrom, userTo), (Integer)1));
            }
        }
    }
    ...
    public THashSet<FriendRequest> getFriendRequests() {
        THashSet<FriendRequest> tHashSet = this.friendRequests;
    ...
    public FriendRequest findFriendRequest(String username) {
        ...
        synchronized (tHashSet) {
            for (FriendRequest friendRequest : this.friendRequests) {
                if (!friendRequest.getUsername().equalsIgnoreCase(username)) continue;
     ...
     */
    public void dispose() {
        Serializable serializable = this.friends;
        synchronized (serializable) {
            this.friends.clear();
        }
        serializable = this.friendRequests;
        synchronized (serializable) {
