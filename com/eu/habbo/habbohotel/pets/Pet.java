...
import com.eu.habbo.habbohotel.pets.PetGestures;
...
Runnable {
    ...
    public Pet(ResultSet set) throws SQLException {
        ...
        if (this.petData == null) {
            LOGGER.error("WARNING! Missing pet data for type: {}! Insert a new entry into the pet_actions table for this type!", (Object)set.getInt("type"));
    ...
    public Pet(int type, int race, String color, String name, int userId) {
        ...
        if (this.petData == null) {
            LOGGER.warn("Missing pet data for type: {}! Insert a new entry into the pet_actions table for this type!", (Object)type);
        }
        this.race = race;
        this.color = color;
        this.experience = 0;
                ...
                try (Connection connection = Emulator.getDatabase().getDataSource().getConnection();){
                    if (this.id > 0) {
                        try (PreparedStatement statement = connection.prepareStatement("UPDATE users_pets SET room_id = ?, experience = ?, energy = ?, respect = ?, x = ?, y = ?, z = ?, rot = ?, hunger = ?, thirst = ?, happiness = ?, created = ? WHERE id = ?");){
                            ...
                            statement.setInt(5, this.roomUnit != null ? (int)this.roomUnit.getX() : 0);
                            statement.setInt(6, this.roomUnit != null ? (int)this.roomUnit.getY() : 0);
                    ...
                    if (this.id != 0) break block30;
                    try (PreparedStatement statement = connection.prepareStatement("INSERT INTO users_pets (user_id, room_id, name, race, type, color, experience, energy, respect, created) VALUES (?, 0, ?, ?, ?, ?, 0, 0, 0, ?)", 1);){
    ...
    public void cycle() {
        ...
        if (this.roomUnit != null && this.task != PetTasks.RIDE) {
            ...
            if (!this.roomUnit.isWalking()) {
                RoomTile tile;
                if (this.roomUnit.getWalkTimeOut() < time && this.canWalk() && (tile = this.room.getRandomWalkableTile()) != null) {
            ...
            if (!this.muted && this.chatTimeout <= time) {
                ...
                this.chatTimeout = time + ((timeOut = Emulator.getRandom().nextInt(30)) < 3 ? 30 : timeOut);
            }
        }
    }
    ...
    public void clearPosture() {
        ...
        if (this.task == null) {
            ...
            for (Map.Entry entry : keys.entrySet()) {
                this.roomUnit.setStatus((RoomUnitStatus)((Object)entry.getKey()), (String)entry.getValue());
            }
            if (!keys.isEmpty()) {
                this.packetUpdate = true;
            }
