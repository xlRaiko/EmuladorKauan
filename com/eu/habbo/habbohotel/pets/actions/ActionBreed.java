...
extends PetAction {
    ...
    public boolean apply(Pet pet, Habbo habbo, String[] data) {
        ...
        for (HabboItem item : pet.getRoom().getRoomSpecialTypes().getItemsOfType(InteractionPetBreedingNest.class)) {
            if (!StringUtils.containsIgnoreCase(item.getBaseItem().getName(), pet.getPetData().getName()) || ((InteractionPetBreedingNest)item).boxFull()) continue;
