...
implements Comparable<PetData> {
    ...
    public HabboItem randomNest(THashSet<InteractionNest> items) {
        ArrayList<InteractionNest> nestList = new ArrayList<InteractionNest>();
        for (InteractionNest nest : items) {
            if (!this.haveNest(nest)) continue;
            nestList.add(nest);
        }
        if (!nestList.isEmpty()) {
            Collections.shuffle(nestList);
            return (HabboItem)nestList.get(0);
        }
        return null;
    }
    ...
    public HabboItem randomFoodItem(THashSet<InteractionPetFood> items) {
        ArrayList<InteractionPetFood> foodList = new ArrayList<InteractionPetFood>();
        for (InteractionPetFood food : items) {
            if (!this.haveFoodItem(food)) continue;
            foodList.add(food);
        }
        if (!foodList.isEmpty()) {
            Collections.shuffle(foodList);
            return (HabboItem)foodList.get(0);
        }
        return null;
    }
    ...
    public HabboItem randomDrinkItem(THashSet<InteractionPetDrink> items) {
        ArrayList<InteractionPetDrink> drinkList = new ArrayList<InteractionPetDrink>();
        for (InteractionPetDrink drink : items) {
            if (!this.haveDrinkItem(drink)) continue;
            drinkList.add(drink);
        }
        if (!drinkList.isEmpty()) {
            Collections.shuffle(drinkList);
            return (HabboItem)drinkList.get(0);
        }
        return null;
    }
    ...
    public HabboItem randomToyItem(THashSet<InteractionPetToy> toys) {
        ArrayList<InteractionPetToy> toyList = new ArrayList<InteractionPetToy>();
        for (InteractionPetToy toy : toys) {
            if (!this.haveToyItem(toy)) continue;
            toyList.add(toy);
        }
        if (!toyList.isEmpty()) {
            Collections.shuffle(toyList);
            return (HabboItem)toyList.get(0);
        }
        return null;
    }

    public PetVocal randomVocal(PetVocalsType type) {
        ArrayList vocals = new ArrayList();
        if (this.petVocals.get((Object)type) != null) {
            vocals.addAll(this.petVocals.get((Object)type));
        }
        if (generalPetVocals.get((Object)type) != null) {
            vocals.addAll(generalPetVocals.get((Object)type));
        }
        if (vocals.isEmpty()) {
            return null;
        }
        return (PetVocal)vocals.get(Emulator.getRandom().nextInt(vocals.size()));
    }
