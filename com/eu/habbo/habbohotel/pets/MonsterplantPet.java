...
implements IPetLook {
    ...
    public static final Map<Integer, Pair<String, Integer>> bodyRarity = new LinkedHashMap<Integer, Pair<String, Integer>>(){
        {
            this.put(1, new Pair<String, Integer>("Blungon", 0));
            this.put(2, new Pair<String, Integer>("Wailzor", 1));
            this.put(3, new Pair<String, Integer>("Stumpy", 1));
            this.put(4, new Pair<String, Integer>("Sunspike", 2));
            this.put(5, new Pair<String, Integer>("Squarg", 0));
            this.put(6, new Pair<String, Integer>("Shroomer", 3));
            this.put(7, new Pair<String, Integer>("Zuchinu", 3));
            this.put(8, new Pair<String, Integer>("Abysswirl", 5));
            this.put(9, new Pair<String, Integer>("Weggylum", 2));
            this.put(10, new Pair<String, Integer>("Wystique", 4));
            this.put(11, new Pair<String, Integer>("Hairbullis", 4));
            this.put(12, new Pair<String, Integer>("Snozzle", 5));
        }
    };
    public static final Map<Integer, Pair<String, Integer>> colorRarity = new LinkedHashMap<Integer, Pair<String, Integer>>(){
        {
            this.put(0, new Pair<String, Integer>("Aenueus", 0));
            this.put(1, new Pair<String, Integer>("Griseus", 1));
            this.put(2, new Pair<String, Integer>("Phoenicus", 2));
            this.put(3, new Pair<String, Integer>("Viridulus", 1));
            this.put(4, new Pair<String, Integer>("Cyaneus", 5));
            this.put(5, new Pair<String, Integer>("Incarnatus", 2));
            this.put(6, new Pair<String, Integer>("Azureus", 4));
            this.put(7, new Pair<String, Integer>("Atamasc", 4));
            this.put(8, new Pair<String, Integer>("Amethyst", 3));
            this.put(9, new Pair<String, Integer>("Fulvus", 0));
            this.put(10, new Pair<String, Integer>("Cinereus", 3));
        }
    };
    public static final ArrayList<Pair<String, Integer>> indexedBody = new ArrayList<Pair<String, Integer>>(bodyRarity.values());
    public static final ArrayList<Pair<String, Integer>> indexedColors = new ArrayList<Pair<String, Integer>>(colorRarity.values());
    ...
    public void run() {
        if (this.needsUpdate) {
            ...
            try (Connection connection = Emulator.getDatabase().getDataSource().getConnection();
                 PreparedStatement statement = connection.prepareStatement("UPDATE users_pets SET mp_type = ?, mp_color = ?, mp_nose = ?, mp_eyes = ?, mp_mouth = ?, mp_nose_color = ?, mp_eyes_color = ?, mp_mouth_color = ?, mp_death_timestamp = ?, mp_breedable = ?, mp_allow_breed = ?, mp_is_dead = ? WHERE id = ?");){
            ...
            if (this.isDead()) {
                ...
                if (difference >= growTime) {
                    ...
                    for (RoomUnitStatus s : this.roomUnit.getStatusMap().keySet()) {
                        if (!s.equals((Object)RoomUnitStatus.GROW)) continue;
                        clear = true;
                    }
                    if (clear) {
                        this.roomUnit.clearStatus();
                    ...
                    int g = (int)Math.ceil((double)difference / ((double)growTime / 7.0));
    ...
    public void breed(MonsterplantPet pet) {
        if (this.canBreed && pet.canBreed) {
            ...
            if ((seedBase = this.getRarity() < 8 || pet.getRarity() < 8 || Emulator.getRandom().nextInt(100) > this.getRarity() + pet.getRarity() ? Emulator.getGameEnvironment().getItemManager().getItem(Emulator.getConfig().getInt("monsterplant.seed.item_id")) : Emulator.getGameEnvironment().getItemManager().getItem(Emulator.getConfig().getInt("monsterplant.seed_rare.item_id"))) != null) {
                HabboItem seed;
                if (ownerOne != null) {
    ...
    private boolean mayScratch() {
        ...
        return (double)(energy / (energyMax = (float)this.getMaxEnergy())) < 0.98;
    }
