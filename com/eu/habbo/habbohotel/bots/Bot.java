...
implements Runnable {
    ...
    public void run() {
        if (this.needsUpdate) {
            try (Connection connection = Emulator.getDatabase().getDataSource().getConnection();
                 PreparedStatement statement = connection.prepareStatement("UPDATE bots SET name = ?, motto = ?, figure = ?, gender = ?, user_id = ?, room_id = ?, dance = ?, freeroam = ?, chat_lines = ?, chat_auto = ?, chat_random = ?, chat_delay = ?, effect = ?, bubble_id = ? WHERE id = ?");){
    ...
    public void cycle(boolean allowBotsWalk) {
        if (this.roomUnit != null) {
            if (allowBotsWalk && this.canWalk && !this.roomUnit.isWalking() && this.roomUnit.getWalkTimeOut() < Emulator.getIntUnixTimestamp() && this.followingHabboId == 0) {
                this.roomUnit.setGoalLocation(BOT_LIMIT_WALKING_DISTANCE ? this.room.getRandomWalkableTilesAround(this.getRoomUnit(), this.room.getLayout().getTile(this.roomUnit.getX(), this.roomUnit.getY()), BOT_WALKING_DISTANCE_RADIUS) : this.room.getRandomWalkableTile());
    ...
    public void talk(String message) {
        if (this.room != null) {
            BotTalkEvent event = new BotTalkEvent(this, message);
            if (((BotChatEvent)Emulator.getPluginManager().fireEvent(event)).isCancelled()) {
                return;
            }
    ...
    public void shout(String message) {
        if (this.room != null) {
            BotShoutEvent event = new BotShoutEvent(this, message);
            if (((BotChatEvent)Emulator.getPluginManager().fireEvent(event)).isCancelled()) {
                return;
            }
            this.chatTimestamp = Emulator.getIntUnixTimestamp();
    ...
    public void clearChat() {
        ArrayList<String> arrayList = this.chatLines;
    ...
    public void onPlaceUpdate() {
        try (Connection connection = Emulator.getDatabase().getDataSource().getConnection();
             PreparedStatement statement = connection.prepareStatement("UPDATE bots SET name = ?, motto = ?, figure = ?, gender = ?, user_id = ?, room_id = ?, x = ?, y = ?, z = ?, rot = ?, dance = ?, freeroam = ?, chat_lines = ?, chat_auto = ?, chat_random = ?, chat_delay = ?, effect = ?, bubble_id = ? WHERE id = ?");){
            statement.setString(1, this.name);
            statement.setString(2, this.motto);
            statement.setString(3, this.figure);
            statement.setString(4, this.gender.toString());
            statement.setInt(5, this.ownerId);
            statement.setInt(6, this.room == null ? 0 : this.room.getId());
            statement.setInt(7, this.roomUnit == null ? 0 : (int)this.roomUnit.getX());
            statement.setInt(8, this.roomUnit == null ? 0 : (int)this.roomUnit.getY());
