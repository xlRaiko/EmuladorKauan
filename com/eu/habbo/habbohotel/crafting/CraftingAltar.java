...
public class CraftingAltar {
    ...
    public Map<CraftingRecipe, Boolean> matchRecipes(Map<Item, Integer> amountMap) {
        THashMap<CraftingRecipe, Boolean> foundRecepies = new THashMap<CraftingRecipe, Boolean>(Math.max(1, this.recipes.size() / 3));
        for (Map.Entry<Integer, CraftingRecipe> set : this.recipes.entrySet()) {
            boolean contains = true;
            if (set.getValue().isLimited() && !set.getValue().canBeCrafted()) continue;
            ...
            for (Map.Entry<Item, Integer> entry : amountMap.entrySet()) {
                ...
                if (set.getValue().getIngredients().containsKey(entry.getKey())) {
                    if (set.getValue().getIngredients().get(entry.getKey()).equals(entry.getValue())) continue;
                    equals = false;
                    if (set.getValue().getIngredients().get(entry.getKey()) > entry.getValue()) continue;
                }
                contains = false;
            }
            if (!contains) continue;
    ...
    public CraftingRecipe getRecipe(String name) {
        for (Map.Entry<Integer, CraftingRecipe> set : this.recipes.entrySet()) {
            if (!set.getValue().getName().equals(name)) continue;
            return set.getValue();
        }
        return null;
    }
    ...
    public CraftingRecipe getRecipe(Map<Item, Integer> items) {
        for (Map.Entry<Integer, CraftingRecipe> set : this.recipes.entrySet()) {
            ...
            for (Map.Entry<Item, Integer> entry : recipe.getIngredients().entrySet()) {
                if (items.containsKey(entry.getKey()) && items.get(entry.getKey()).equals(entry.getValue())) continue;
                recipe = null;
                break;
            }
    ...
    public List<CraftingRecipe> getRecipesForHabbo(Habbo habbo) {
        ...
        for (Map.Entry<Integer, CraftingRecipe> set : this.recipes.entrySet()) {
            if (set.getValue().isSecret() && !habbo.getHabboStats().hasRecipe(set.getValue().getId())) continue;
