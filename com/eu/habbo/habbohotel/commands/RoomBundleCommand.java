...
public class RoomBundleCommand
extends Command {
    ...
    public boolean handle(GameClient gameClient, String[] params) throws Exception {
        ...
        int parentId = Integer.parseInt(params[1]);
        int credits = Integer.parseInt(params[2]);
        int points = Integer.parseInt(params[3]);
        int pointsType = Integer.parseInt(params[4]);
        ...
        if (page instanceof RoomBundleLayout) {
            block36: {
                try (Connection connection = Emulator.getDatabase().getDataSource().getConnection();
                     PreparedStatement statement = connection.prepareStatement("INSERT INTO catalog_items (page_id, item_ids, catalog_name, cost_credits, cost_points, points_type ) VALUES (?, ?, ?, ?, ?, ?)", 1);){
