...
public class SummonRankCommand
extends Command {
    ...
    public boolean handle(GameClient gameClient, String[] params) throws Exception {
        if (params.length >= 2) {
            ...
            for (Map.Entry<Integer, Habbo> set : Emulator.getGameEnvironment().getHabboManager().getOnlineHabbos().entrySet()) {
                if (set.getValue().getHabboInfo().getRank().getId() < minRank || set.getValue() == gameClient.getHabbo() || set.getValue().getHabboInfo().getCurrentRoom() == gameClient.getHabbo().getHabboInfo().getCurrentRoom()) continue;
