...
extends Command {
    ...
    public boolean handle(GameClient gameClient, String[] params) throws Exception {
        ...
        ConcurrentHashMap<Integer, Habbo> concurrentHashMap = Emulator.getGameEnvironment().getHabboManager().getOnlineHabbos();
        synchronized (concurrentHashMap) {
            ArrayList<Habbo> staffs = new ArrayList<Habbo>();
            for (Map.Entry<Integer, Habbo> set : Emulator.getGameEnvironment().getHabboManager().getOnlineHabbos().entrySet()) {
                if (set.getValue().getHabboInfo().getRank().getId() < minRank) continue;
