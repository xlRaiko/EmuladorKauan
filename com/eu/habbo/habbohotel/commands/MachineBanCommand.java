...
public class MachineBanCommand
extends Command {
    ...
    public boolean handle(GameClient gameClient, String[] params) throws Exception {
        ...
        if (params.length >= 2) {
            ...
            habbo = h != null ? h.getHabboInfo() : HabboManager.getOfflineHabboInfo(params[1]);
        }
        if (params.length > 2) {
        ...
        int count = Emulator.getGameEnvironment().getModToolManager().ban(habbo.getId(), gameClient.getHabbo(), reason.toString(), 315569260, ModToolBanType.MACHINE, -1).size();
