...
public class EmptyPetsInventoryCommand
extends Command {
    ...
    public boolean handle(GameClient gameClient, String[] params) throws Exception {
        ...
        if (params.length >= 2 && params[1].equalsIgnoreCase(Emulator.getTexts().getValue("generic.yes"))) {
            Habbo habbo;
            Habbo habbo2 = habbo = params.length == 3 && gameClient.getHabbo().hasPermission(Permission.ACC_EMPTY_OTHERS) ? Emulator.getGameEnvironment().getHabboManager().getHabbo(params[2]) : gameClient.getHabbo();
            if (habbo != null) {
                TIntObjectHashMap<Pet> pets = new TIntObjectHashMap<Pet>();
                ...
                pets.forEachValue(object -> {
                    Emulator.getGameEnvironment().getPetManager().deletePet((Pet)object);
                    return true;
                });
