...
extends Command {
    ...
    public boolean handle(GameClient gameClient, String[] params) throws Exception {
        ...
        HabboInfo habboInfo = habbo = onlineHabbo != null ? onlineHabbo.getHabboInfo() : null;
        if (habbo == null) {
        ...
        StringBuilder message = new StringBuilder(Emulator.getTexts().getValue("command.cmd_userinfo.userinfo") + ":  <b>" + habbo.getUsername() + "</b> (<b>" + habbo.getId() + "</b>)\r" + Emulator.getTexts().getValue("command.cmd_userinfo.user_id") + ": " + habbo.getId() + "\r" + Emulator.getTexts().getValue("command.cmd_userinfo.user_name") + ": " + habbo.getUsername() + "\r" + Emulator.getTexts().getValue("command.cmd_userinfo.motto") + ": " + habbo.getMotto().replace("<", "[").replace(">", "]") + "\r" + Emulator.getTexts().getValue("command.cmd_userinfo.rank") + ": " + habbo.getRank().getName() + " (" + habbo.getRank().getId() + ") \r" + Emulator.getTexts().getValue("command.cmd_userinfo.online") + ": " + (onlineHabbo == null ? Emulator.getTexts().getValue("generic.no") : Emulator.getTexts().getValue("generic.yes")) + "\r" + (String)(habbo.getRank().hasPermission(Permission.ACC_HIDE_MAIL, true) ? "" : Emulator.getTexts().getValue("command.cmd_userinfo.email") + ": " + habbo.getMail() + "\r") + (String)(habbo.getRank().hasPermission(Permission.ACC_HIDE_IP, true) ? "" : Emulator.getTexts().getValue("command.cmd_userinfo.ip_register") + ": " + habbo.getIpRegister() + "\r") + (String)(habbo.getRank().hasPermission(Permission.ACC_HIDE_IP, true) || onlineHabbo == null ? "" : Emulator.getTexts().getValue("command.cmd_userinfo.ip_current") + ": " + onlineHabbo.getHabboInfo().getIpLogin() + "\r") + (String)(onlineHabbo != null ? Emulator.getTexts().getValue("command.cmd_userinfo.achievement_score") + ": " + onlineHabbo.getHabboStats().achievementScore + "\r" : ""));
        ...
        int i = habbo.getCurrencies().size();
        while (i-- > 0) {
            try {
                iterator.advance();
            }
        ...
        message.append("\r").append((String)(onlineHabbo != null ? "<b>" + Emulator.getTexts().getValue("command.cmd_userinfo.current_activity") + "</b>\r" : "")).append((String)(onlineHabbo != null ? Emulator.getTexts().getValue("command.cmd_userinfo.room") + ": " + (String)(onlineHabbo.getHabboInfo().getCurrentRoom() != null ? onlineHabbo.getHabboInfo().getCurrentRoom().getName() + "(" + onlineHabbo.getHabboInfo().getCurrentRoom().getId() + ")\r" : "-") : "")).append((String)(onlineHabbo != null ? Emulator.getTexts().getValue("command.cmd_userinfo.respect_left") + ": " + onlineHabbo.getHabboStats().respectPointsToGive + "\r" : "")).append((String)(onlineHabbo != null ? Emulator.getTexts().getValue("command.cmd_userinfo.pet_respect_left") + ": " + onlineHabbo.getHabboStats().petRespectPointsToGive + "\r" : "")).append((String)(onlineHabbo != null ? Emulator.getTexts().getValue("command.cmd_userinfo.allow_trade") + ": " + (onlineHabbo.getHabboStats().allowTrade() ? Emulator.getTexts().getValue("generic.yes") : Emulator.getTexts().getValue("generic.no")) + "\r" : "")).append((String)(onlineHabbo != null ? Emulator.getTexts().getValue("command.cmd_userinfo.allow_follow") + ": " + (onlineHabbo.getHabboStats().blockFollowing ? Emulator.getTexts().getValue("generic.no") : Emulator.getTexts().getValue("generic.yes")) + "\r" : "")).append((String)(onlineHabbo != null ? Emulator.getTexts().getValue("command.cmd_userinfo.allow_friend_request") + ": " + (onlineHabbo.getHabboStats().blockFriendRequests ? Emulator.getTexts().getValue("generic.no") : Emulator.getTexts().getValue("generic.yes")) + "\r" : ""));
        ...
        if (!nameChanges.isEmpty()) {
            ...
            for (Map.Entry<Integer, String> entry : nameChanges) {
                message.append(format.format(new Date((long)entry.getKey().intValue() * 1000L))).append(" : ").append(entry.getValue()).append("<br/>");
            }
        }
        if (onlineHabbo != null) {
