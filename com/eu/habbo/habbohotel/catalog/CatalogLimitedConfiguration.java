...
public class CatalogLimitedConfiguration
implements Runnable {
    ...
    public int getNumber() {
        LinkedList<Integer> linkedList = this.limitedNumbers;
    ...
    public void limitedSold(int catalogItemId, Habbo habbo, HabboItem item) {
        ...
        synchronized (linkedList) {
            try (Connection connection = Emulator.getDatabase().getDataSource().getConnection();
                 PreparedStatement statement = connection.prepareStatement("UPDATE catalog_items_limited SET user_id = ?, timestamp = ?, item_id = ? WHERE catalog_item_id = ? AND number = ? AND user_id = 0 LIMIT 1");){
