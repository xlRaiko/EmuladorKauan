...
Comparable<CatalogItem> {
    ...
    public static boolean haveOffer(CatalogItem item) {
        ...
        for (Item i : item.getBaseItems()) {
            if (!i.getName().toLowerCase().startsWith("cf_") && !i.getName().toLowerCase().startsWith("cfc_") && !i.getName().toLowerCase().startsWith("rentable_bot")) continue;
            return false;
        }
    ...
    public THashSet<Item> getBaseItems() {
        ...
        if (!this.itemId.isEmpty()) {
            String[] itemIds;
            for (String itemId : itemIds = this.itemId.split(";")) {
                Item item;
                int identifier;
                ...
                try {
                    ...
                    LOGGER.info("Invalid value ({}) for items_base column for catalog_item id ({}). Value must be integer or of the format of integer:amount;integer:amount", (Object)itemId, (Object)this.id);
                    continue;
                }
                if (identifier <= 0 || (item = Emulator.getGameEnvironment().getItemManager().getItem(identifier)) == null) continue;
                items.add(item);
            }
        }
        return items;
            ...
            try {
                ...
                for (String itemId : itemIds = this.itemId.split(";")) {
                    if (itemId.contains(":")) {
                        ...
                        if (data.length <= 1 || Integer.parseInt(data[0]) <= 0 || Integer.parseInt(data[1]) <= 0) continue;
                ...
                LOGGER.debug("Failed to load {}", (Object)this.itemId);
    ...
    public void serialize(ServerMessage message) {
        ...
        for (Item item : items) {
            ...
            if (this.getName().contains("wallpaper_single") || this.getName().contains("floor_single") || this.getName().contains("landscape_single")) {
                ...
                for (String s : this.getExtradata().split(";")) {
                    if (!s.startsWith("figure:")) continue;
                    lookFound = true;
        ...
        message.appendBoolean(CatalogItem.haveOffer(this));
        message.appendBoolean(false);
