...
public class MarketPlace {
    ...
    public static THashSet<MarketPlaceOffer> getOwnOffers(Habbo habbo) {
        ...
        try (Connection connection = Emulator.getDatabase().getDataSource().getConnection();
             PreparedStatement statement = connection.prepareStatement("SELECT items_base.type AS type, items.item_id AS base_item_id, items.limited_data AS ltd_data, marketplace_items.* FROM marketplace_items INNER JOIN items ON marketplace_items.item_id = items.id INNER JOIN items_base ON items.item_id = items_base.id WHERE marketplace_items.user_id = ?");){
    ...
    public static void takeBackItem(Habbo habbo, int offerId) {
        ...
        if (!Emulator.getPluginManager().fireEvent(new MarketPlaceItemCancelledEvent(offer)).isCancelled()) {
            MarketPlace.takeBackItem(habbo, offer);
        }
    }
    ...
    private static void takeBackItem(Habbo habbo, MarketPlaceOffer offer) {
        block50: {
            if (offer != null && habbo.getInventory().getMarketplaceItems().contains(offer)) {
                ...
                try (Connection connection = Emulator.getDatabase().getDataSource().getConnection();
                     PreparedStatement ownerCheck = connection.prepareStatement("SELECT user_id FROM marketplace_items WHERE id = ?", 1004, 1007);){
    ...
    public static List<MarketPlaceOffer> getOffers(int minPrice, int maxPrice, String search, int sort) {
        ArrayList<MarketPlaceOffer> offers = new ArrayList<MarketPlaceOffer>(10);
        Object query = "SELECT B.* FROM marketplace_items a INNER JOIN (SELECT b.item_id AS base_item_id, b.limited_data AS ltd_data, marketplace_items.*, AVG(price) as avg, MIN(marketplace_items.price) as minPrice, MAX(marketplace_items.price) as maxPrice, COUNT(*) as number, (SELECT COUNT(*) FROM marketplace_items c INNER JOIN items as items_b ON c.item_id = items_b.id WHERE state = 2 AND items_b.item_id = base_item_id AND DATE(from_unixtime(sold_timestamp)) = CURDATE()) as sold_count_today FROM marketplace_items INNER JOIN items b ON marketplace_items.item_id = b.id INNER JOIN items_base bi ON b.item_id = bi.id INNER JOIN catalog_items ci ON bi.id = ci.item_ids WHERE price = (SELECT MIN(e.price) FROM marketplace_items e, items d WHERE e.item_id = d.id AND d.item_id = b.item_id AND e.state = 1 AND e.timestamp > ? GROUP BY d.item_id) AND state = 1 AND timestamp > ?";
        if (minPrice > 0) {
            query = (String)query + " AND CEIL(price + (price / 100)) >= " + minPrice;
        }
        if (maxPrice > 0 && maxPrice > minPrice) {
            query = (String)query + " AND CEIL(price + (price / 100)) <= " + maxPrice;
        }
        if (search.length() > 0) {
            query = (String)query + " AND ( bi.public_name LIKE ? OR ci.catalog_name LIKE ? ) ";
        }
        query = (String)query + " GROUP BY base_item_id, ltd_data";
        switch (sort) {
            case 6: {
                query = (String)query + " ORDER BY number ASC";
                break;
            }
            case 5: {
                query = (String)query + " ORDER BY number DESC";
                break;
            }
            case 4: {
                query = (String)query + " ORDER BY sold_count_today ASC";
                break;
            }
            case 3: {
                query = (String)query + " ORDER BY sold_count_today DESC";
                break;
            }
            case 2: {
                query = (String)query + " ORDER BY minPrice ASC";
                break;
            }
            default: {
                query = (String)query + " ORDER BY minPrice DESC";
            }
        }
        query = (String)query + ")";
        query = (String)query + " AS B ON a.id = B.id";
        query = (String)query + " LIMIT 250";
        try (Connection connection = Emulator.getDatabase().getDataSource().getConnection();
             PreparedStatement statement = connection.prepareStatement((String)query);){
    ...
    public static void serializeItemInfo(int itemId, ServerMessage message) {
        try (Connection connection = Emulator.getDatabase().getDataSource().getConnection();
             PreparedStatement statement = connection.prepareStatement("SELECT avg(marketplace_items.price) as price, COUNT(*) as sold, (datediff(NOW(), DATE(from_unixtime(marketplace_items.timestamp)))) as day FROM marketplace_items INNER JOIN items ON items.id = marketplace_items.item_id INNER JOIN items_base ON items.item_id = items_base.id WHERE items.limited_data = '0:0' AND marketplace_items.state = 2 AND items_base.sprite_id = ? AND DATE(from_unixtime(marketplace_items.timestamp)) >= NOW() - INTERVAL 30 DAY GROUP BY DATE(from_unixtime(marketplace_items.timestamp))", 1004, 1007);){
            statement.setInt(1, itemId);
            message.appendInt(MarketPlace.avarageLastXDays(itemId, 7));
            message.appendInt(MarketPlace.itemsOnSale(itemId));
    ...
    public static int itemsOnSale(int baseItemId) {
        int number = 0;
        try (Connection connection = Emulator.getDatabase().getDataSource().getConnection();
             PreparedStatement statement = connection.prepareStatement("SELECT COUNT(*) as number, AVG(price) as avg FROM marketplace_items INNER JOIN items ON marketplace_items.item_id = items.id INNER JOIN items_base ON items.item_id = items_base.id WHERE state = 1 AND timestamp >= ? AND items_base.sprite_id = ?", 1004, 1007);){
            statement.setInt(1, Emulator.getIntUnixTimestamp() - 172800);
    ...
    private static int avarageLastXDays(int baseItemId, int days) {
        ...
        return MarketPlace.calculateCommision(avg);
    }
    ...
    public static void buyItem(int offerId, GameClient client) {
        block49: {
            ...
            try (Connection connection = Emulator.getDatabase().getDataSource().getConnection();
                ...
                try (ResultSet set = statement.executeQuery();){
                    if (!set.next()) break block49;
                    try (PreparedStatement itemStatement = connection.prepareStatement("SELECT * FROM items WHERE id = ? LIMIT 1", 1004, 1007);){
                        itemStatement.setInt(1, set.getInt("item_id"));
                        try (ResultSet itemSet = itemStatement.executeQuery();){
                            itemSet.first();
                            if (itemSet.getRow() <= 0) break block49;
                            int price = MarketPlace.calculateCommision(set.getInt("price"));
                            if (set.getInt("state") != 1) {
                                MarketPlace.sendErrorMessage(client, set.getInt("item_id"), offerId);
                                break block49;
                            }
                            ...
                            event.price = MarketPlace.calculateCommision(event.price);
        ...
        try (Connection connection = Emulator.getDatabase().getDataSource().getConnection();
            ...
            try (ResultSet countSet = statement.executeQuery();){
                ...
                if (countSet.getRow() == 0) {
                    ...
                    countSet.first();
                    client.sendResponse(new MarketplaceBuyErrorComposer(3, countSet.getInt("count"), countSet.getInt("id"), MarketPlace.calculateCommision(countSet.getInt("price"))));
                }
            }
        }
        catch (SQLException e) {
    ...
    public static void getCredits(GameClient client) {
        ...
        HabboInventory habboInventory = client.getHabbo().getInventory();
        synchronized (habboInventory) {
