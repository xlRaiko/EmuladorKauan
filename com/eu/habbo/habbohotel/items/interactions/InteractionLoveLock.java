...
public class InteractionLoveLock
extends HabboItem {
    ...
    public void onClick(GameClient client, Room room, Object[] objects) throws Exception {
        ...
        if (RoomLayout.tilesAdjecent(client.getHabbo().getRoomUnit().getCurrentLocation(), room.getLayout().getTile(this.getX(), this.getY()))) {
            ...
            } else if (this.userOneId != client.getHabbo().getHabboInfo().getId() && (habbo = room.getHabbo(this.userOneId)) != null) {
    ...
    public boolean lock(Habbo userOne, Habbo userTwo, Room room) {
        ...
        if (RoomLayout.tilesAdjecent(userOne.getRoomUnit().getCurrentLocation(), tile) && RoomLayout.tilesAdjecent(userTwo.getRoomUnit().getCurrentLocation(), tile)) {
            Object data = "1";
            data = (String)data + "\t";
            data = (String)data + userOne.getHabboInfo().getUsername();
            data = (String)data + "\t";
            data = (String)data + userTwo.getHabboInfo().getUsername();
            data = (String)data + "\t";
            data = (String)data + userOne.getHabboInfo().getLook();
            data = (String)data + "\t";
            data = (String)data + userTwo.getHabboInfo().getLook();
            data = (String)data + "\t";
            data = (String)data + Calendar.getInstance().get(5) + "-" + (Calendar.getInstance().get(2) + 1) + "-" + Calendar.getInstance().get(1);
            this.setExtradata((String)data);
