...
public class InteractionPetBreedingNest
extends HabboItem {
    ...
    public void stopBreeding(Habbo habbo) {
        ...
        if (this.petTwo.getUserId() != habbo.getHabboInfo().getId() && (owner = this.petTwo.getRoom().getHabbo(this.petTwo.getUserId())) != null) {
    ...
    public void breed(Habbo habbo, String name, int petOneId, int petTwoId) {
        ...
        this.setExtradata("2");
        habbo.getHabboInfo().getCurrentRoom().updateItem(this);
        InteractionPetBreedingNest box = this;
        Pet petOne = this.petOne;
        Pet petTwo = this.petTwo;
        Emulator.getThreading().run(() -> {
            Pet offspring = Emulator.getGameEnvironment().getPetManager().createPet(petOne.getPetData().getOffspringType(), (int)Math.min(Math.round(Math.max(1.0, PetManager.getNormalDistributionForBreeding(petOne.getLevel(), petTwo.getLevel()).sample())), 20L), name, habbo.getClient());
