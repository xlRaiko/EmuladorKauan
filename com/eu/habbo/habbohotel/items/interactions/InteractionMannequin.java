...
public class InteractionMannequin
extends HabboItem {
    ...
    public void onClick(GameClient client, Room room, Object[] objects) throws Exception {
        ...
        for (String playerFigurePart : client.getHabbo().getHabboInfo().getLook().split("\\.")) {
            if (playerFigurePart.startsWith("ch") || playerFigurePart.startsWith("lg")) continue;
            newFigure = (String)newFigure + playerFigurePart + ".";
        }
        String newFigureParts = figure;
        for (String newFigurePart : newFigureParts.split("\\.")) {
            if (!newFigurePart.startsWith("hd")) continue;
            newFigureParts = newFigureParts.replace(newFigurePart, "");
        }
        if (newFigureParts.equals("")) {
            return;
        }
        String newLook = (String)newFigure + newFigureParts;
        if (newLook.length() > 512) {
            return;
        }
        client.getHabbo().getHabboInfo().setLook((String)(ClothingValidationManager.VALIDATE_ON_MANNEQUIN ? ClothingValidationManager.validateLook(client.getHabbo(), newLook, client.getHabbo().getHabboInfo().getGender().name()) : newLook));
