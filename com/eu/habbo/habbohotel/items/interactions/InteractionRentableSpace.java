...
public class InteractionRentableSpace
extends HabboItem {
    ...
    public void endRent() {
        ...
        for (HabboItem item : items) {
            if (item.getUserId() != this.renterId) continue;
        ...
        if (habbo != null) {
            ...
            int zero = 0;
            try (Connection connection = Emulator.getDatabase().getDataSource().getConnection();
                 PreparedStatement statement = connection.prepareStatement("UPDATE users_settings SET rent_space_id = ?, rent_space_endtime = ? WHERE user_id = ? LIMIT 1");){
