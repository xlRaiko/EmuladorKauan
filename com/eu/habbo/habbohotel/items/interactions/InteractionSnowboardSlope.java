...
public class InteractionSnowboardSlope
extends InteractionMultiHeight {
    ...
    public void onPlace(Room room) {
        ...
        int progress = habbo != null ? habbo.getHabboStats().getAchievementProgress(snowboardBuild) : AchievementManager.getAchievementProgressForHabbo(room.getOwnerId(), snowboardBuild);
    ...
    public void onPickUp(Room room) {
        for (Habbo habbo : room.getHabbosOnItem(this)) {
            if (habbo.getRoomUnit().getEffectId() != 97) continue;
    ...
    public void onMove(Room room, RoomTile oldLocation, RoomTile newLocation) {
        ...
        for (Habbo habbo : room.getHabbosOnItem(this)) {
            if (habbo.getRoomUnit().getEffectId() != 97 || newRect.contains(habbo.getRoomUnit().getCurrentLocation().x, habbo.getRoomUnit().getCurrentLocation().y)) continue;
            room.giveEffect(habbo, 0, -1);
        }
    }
}
