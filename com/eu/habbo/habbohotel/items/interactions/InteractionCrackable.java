...
public class InteractionCrackable
extends HabboItem {
    ...
    public void onClick(GameClient client, Room room, Object[] objects) throws Exception {
        ...
        synchronized (object) {
            ...
            if (this.getBaseItem().getEffectF() > 0 && client.getHabbo().getHabboInfo().getGender().equals((Object)HabboGender.F) && this.getBaseItem().getEffectF() == client.getHabbo().getRoomUnit().getEffectId()) {
                return;
            }
            if (this.getBaseItem().getEffectM() > 0 && client.getHabbo().getHabboInfo().getGender().equals((Object)HabboGender.M) && this.getBaseItem().getEffectM() == client.getHabbo().getRoomUnit().getEffectId()) {
                return;
            }
    ...
    public void onTick(Habbo habbo, Room room) {
        CrackableReward rewardData;
        if (this.cracked) {
            return;
        }
        if ((this.allowAnyone() || this.getUserId() == habbo.getHabboInfo().getId()) && (rewardData = Emulator.getGameEnvironment().getItemManager().getCrackableData(this.getBaseItem().getId())) != null) {
