...
public class WiredConditionFurniHaveFurni
extends InteractionWiredCondition {
    ...
    public boolean execute(RoomUnit roomUnit, Room room, Object[] stuff) {
        ...
        if (this.all) {
            return this.items.stream().allMatch(item -> {
                ...
                return occupiedTiles.stream().anyMatch(tile -> room.getItemsAt((RoomTile)tile).stream().anyMatch(matchedItem -> matchedItem != item && matchedItem.getZ() >= minZ));
            });
        }
    ...
    public String getWiredData() {
        this.refresh();
        return WiredHandler.getGsonBuilder().create().toJson(new JsonData(this.all, this.items.stream().map(HabboItem::getId).collect(Collectors.toList())));
    }
