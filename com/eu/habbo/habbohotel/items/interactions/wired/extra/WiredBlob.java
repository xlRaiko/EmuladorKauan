...
extends InteractionDefault {
    ...
    public void onWalkOn(RoomUnit roomUnit, Room room, Object[] objects) throws Exception {
        ...
        if (!this.getExtradata().equals(WiredBlobState.ACTIVE.getState())) {
            return;
        }
        Habbo habbo = room.getHabbo(roomUnit);
        if (habbo != null && (player = habbo.getHabboInfo().getGamePlayer()) != null) {
    ...
    public void onClick(GameClient client, Room room, Object[] objects) throws Exception {
        if (!this.RESETS_WITH_GAME && objects != null && objects.length == 2 && objects[1].equals((Object)WiredEffectType.TOGGLE_STATE) && room.getGames().stream().anyMatch(game -> game.getState().equals((Object)GameState.RUNNING) || game.getState().equals((Object)GameState.PAUSED))) {
    ...
    static enum WiredBlobState {
        ACTIVE("0"),
        USED("1");

        private String state;

        private WiredBlobState(String state) {
            this.state = state;
        }
