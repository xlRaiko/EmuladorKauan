...
public class WiredEffectWhisper
extends InteractionWiredEffect {
    ...
    public void serializeWiredData(ServerMessage message, Room room) {
        ...
        message.appendInt(0);
        message.appendInt(0);
        message.appendInt(WiredEffectWhisper.type.code);
    ...
    public boolean execute(RoomUnit roomUnit, Room room, Object[] stuff) {
        ...
        if (this.message.length() > 0 && roomUnit != null && (habbo = room.getHabbo(roomUnit)) != null) {
            String msg = this.message.replace("%tag%", habbo.getHabboStats().cacheTag.isEmpty() ? "No tienes ning\u00fan tag" : habbo.getHabboStats().cacheTag).replace("%user%", habbo.getHabboInfo().getUsername()).replace("%online_count%", "" + Emulator.getGameEnvironment().getHabboManager().getOnlineCount()).replace("%room_count%", "" + Emulator.getGameEnvironment().getRoomManager().getActiveRooms().size());
