...
public class WiredEffectTeleportGreen
extends InteractionWiredEffect {
    public static final WiredEffectType type = WiredEffectType.TELEPORT;
    private static final GameTeamColors teamColor = GameTeamColors.GREEN;
    protected List<HabboItem> items = new ArrayList<HabboItem>();

    public WiredEffectTeleportGreen(ResultSet set, Item baseItem) throws SQLException {
        super(set, baseItem);
    }

    public WiredEffectTeleportGreen(int id, int userId, Item item, String extradata, int limitedStack, int limitedSells) {
    ...
    public static void teleportUnitToTile(Room room, RoomTile tile, boolean isAnimation) {
        ...
        for (Habbo habbo : room.getHabbos()) {
            Game game;
            if (habbo.getHabboInfo().getCurrentGame() == null || (game = room.getGame(habbo.getHabboInfo().getCurrentGame())) == null || !game.getTeamForHabbo(habbo).getTeamColor().equals((Object)teamColor) || habbo.getRoomUnit().isWiredTeleporting) continue;
    ...
    public boolean execute(RoomUnit roomUnit, Room room, Object[] stuff) {
        List<HabboItem> executeItems = this.items;
        executeItems.removeIf(item -> item == null || item.getRoomId() != this.getRoomId() || Emulator.getGameEnvironment().getRoomManager().getRoom(this.getRoomId()).getHabboItem(item.getId()) == null);
        if (!executeItems.isEmpty()) {
            int i = Emulator.getRandom().nextInt(executeItems.size());
            HabboItem item2 = executeItems.get(i);
            boolean isNotAnimation = room.getRoomSpecialTypes().hasExtraType(this.getX(), this.getY(), WiredAddonNoAnimationUnit.class);
            WiredEffectTeleportGreen.teleportUnitToTile(room, room.getLayout().getTile(item2.getX(), item2.getY()), isNotAnimation);
            return true;
        }
        return false;
    }
