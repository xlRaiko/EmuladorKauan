...
public class WiredEffectBotTeleport
extends InteractionWiredEffect {
    ...
    public boolean execute(RoomUnit roomUnit, Room room, Object[] stuff) {
        ...
        int j = 1;
        for (HabboItem item : this.items) {
            if (item.getRoomId() == 0 || item.getRoomId() != bot.getRoom().getId()) continue;
            if (i == j) {
                WiredEffectBotTeleport.teleportUnitToTile(bot.getRoomUnit(), room.getLayout().getTile(item.getX(), item.getY()));
                return true;
            }
            ++j;
        }
