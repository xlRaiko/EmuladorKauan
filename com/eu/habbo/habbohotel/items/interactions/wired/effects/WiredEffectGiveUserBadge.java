...
public class WiredEffectGiveUserBadge
extends InteractionWiredEffect {
    public static final WiredEffectType type = WiredEffectType.SHOW_MESSAGE;
    protected String message = "";

    public WiredEffectGiveUserBadge(ResultSet set, Item baseItem) throws SQLException {
        super(set, baseItem);
    }

    public WiredEffectGiveUserBadge(int id, int userId, Item item, String extradata, int limitedStack, int limitedSells) {
    ...
    public void serializeWiredData(ServerMessage message, Room room) {
        ...
        message.appendInt(0);
        message.appendInt(0);
        message.appendInt(WiredEffectGiveUserBadge.type.code);
        message.appendInt(this.getDelay());
        if (this.requiresTriggeringUser()) {
            final ArrayList invalidTriggers = new ArrayList();
            room.getRoomSpecialTypes().getTriggers(this.getX(), this.getY()).forEach(new TObjectProcedure<InteractionWiredTrigger>(){
                final /* synthetic */ WiredEffectGiveUserBadge this$0;
                {
                    this.this$0 = this$0;
                }
    ...
    public boolean execute(RoomUnit roomUnit, Room room, Object[] stuff) {
        ...
        if (this.message.length() > 0 && roomUnit != null && (habbo = room.getHabbo(roomUnit)) != null) {
            if (habbo.getInventory().getBadgesComponent().hasBadge(this.message)) {
                habbo.whisper(Emulator.getTexts().getValue("phbplugin.wireds.givebadge.error"));
            } else {
                HabboBadge badge = new HabboBadge(0, this.message, 0, habbo);
                badge.run();
                habbo.getInventory().getBadgesComponent().addBadge(badge);
                habbo.getClient().sendResponse(new AddUserBadgeComposer(badge));
                habbo.whisper(Emulator.getTexts().getValue("phbplugin.wireds.givebadge.success"));
            }
        }
        return false;
    }
