...
public class WiredEffectTriggerStacksNegative
extends InteractionWiredEffect {
    ...
    public WiredEffectTriggerStacksNegative(ResultSet set, Item baseItem) throws SQLException {
        super(set, baseItem);
    }

    public WiredEffectTriggerStacksNegative(int id, int userId, Item item, String extradata, int limitedStack, int limitedSells) {
    ...
    public boolean execute(RoomUnit roomUnit, Room room, Object[] stuff) {
        ...
        for (HabboItem item : this.items) {
            ...
            for (RoomTile tile : usedTiles) {
                if (tile.x != item.getX() || tile.y != item.getY()) continue;
                found = true;
                break;
            }
            if (found) continue;
