...
extends InteractionWiredEffect {
    ...
    public boolean execute(RoomUnit roomUnit, Room room, Object[] stuff) {
        ...
        if (habbo != null && habbo.getHabboInfo().getCurrentGame() != null) {
            ...
            TObjectIntHashMap<Map.Entry<Integer, Integer>> dataClone = new TObjectIntHashMap<Map.Entry<Integer, Integer>>(this.data);
            TObjectIntIterator iterator = dataClone.iterator();
            int i = dataClone.size();
            while (i-- > 0) {
                iterator.advance();
                Map.Entry map = (Map.Entry)iterator.key();
                if (((Integer)map.getValue()).intValue() != habbo.getHabboInfo().getId()) continue;
                if ((Integer)map.getKey() == gameStartTime) {
                    if (iterator.value() >= this.count) continue;
            ...
            try {
                this.data.put(new AbstractMap.SimpleEntry<Integer, Integer>(gameStartTime, habbo.getHabboInfo().getId()), 1);
            }
    ...
    public void serializeWiredData(ServerMessage message, Room room) {
        ...
        if (this.requiresTriggeringUser()) {
            ...
            room.getRoomSpecialTypes().getTriggers(this.getX(), this.getY()).forEach(new TObjectProcedure<InteractionWiredTrigger>(){
                final /* synthetic */ WiredEffectGiveScore this$0;
                {
                    this.this$0 = this$0;
                }
