...
public class WiredEffectGiveUserDance
extends InteractionWiredEffect {
    public static final WiredEffectType type = WiredEffectType.SHOW_MESSAGE;
    protected String message = "";

    public WiredEffectGiveUserDance(ResultSet set, Item baseItem) throws SQLException {
        super(set, baseItem);
    }

    public WiredEffectGiveUserDance(int id, int userId, Item item, String extradata, int limitedStack, int limitedSells) {
    ...
    public void serializeWiredData(ServerMessage message, Room room) {
        ...
        message.appendInt(0);
        message.appendInt(0);
        message.appendInt(WiredEffectGiveUserDance.type.code);
    ...
    public boolean execute(RoomUnit roomUnit, Room room, Object[] stuff) {
        ...
        if (this.message.length() > 0 && roomUnit != null && (habbo = room.getHabbo(roomUnit)) != null) {
            Integer dance = Integer.valueOf(this.message);
            DanceType dancetype = DanceType.DUCK_FUNK;
            if (dance.equals(1)) {
                dancetype = DanceType.HAB_HOP;
            } else if (dance.equals(2)) {
                dancetype = DanceType.POGO_MOGO;
            } else if (dance.equals(3)) {
                dancetype = DanceType.DUCK_FUNK;
            } else if (dance.equals(4)) {
                dancetype = DanceType.THE_ROLLIE;
            } else if (dance.equals(0)) {
                dancetype = DanceType.NONE;
            } else {
                habbo.whisper("Digite um n\u00famero de 0 a 4. (0 desativa)");
            }
            habbo.getHabboInfo().getCurrentRoom().dance(habbo, dancetype);
            return true;
        }
        return false;
    }
