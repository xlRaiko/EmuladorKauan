...
public class WiredEffectTopDontMoveFurniTowards
extends InteractionWiredEffect {
    ...
    public WiredEffectTopDontMoveFurniTowards(ResultSet set, Item baseItem) throws SQLException {
        super(set, baseItem);
    }

    public WiredEffectTopDontMoveFurniTowards(int id, int userId, Item item, String extradata, int limitedStack, int limitedSells) {
    ...
    public boolean execute(RoomUnit roomUnit, Room room, Object[] stuff) {
        ...
        for (HabboItem item2 : this.items) {
            room.getHabbos().stream().filter(habbo -> habbo != null && item2.getOccupyingTiles(layout).contains(habbo.getRoomUnit().getCurrentLocation())).forEach(habbo -> Emulator.getThreading().run(new WiredCollissionRunnable(habbo.getRoomUnit(), room, new Object[]{item2})));
        }
        return true;
    }
    ...
    public void serializeWiredData(ServerMessage message, Room room) {
        ...
        for (HabboItem habboItem : this.items) {
            message.appendInt(habboItem.getId());
        }
