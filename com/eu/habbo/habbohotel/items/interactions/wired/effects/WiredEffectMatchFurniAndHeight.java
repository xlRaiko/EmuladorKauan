...
import com.eu.habbo.habbohotel.items.interactions.wired.effects.WiredMatchFurniAndHeightSetting;
...
public class WiredEffectMatchFurniAndHeight
extends InteractionWiredEffect {
    private static final WiredEffectType type = WiredEffectType.MATCH_SSHOT;
    private final THashSet<WiredMatchFurniAndHeightSetting> settings = new THashSet(0);
    private boolean state = false;
    private boolean direction = false;
    private boolean position = false;
    public boolean checkForWiredResetPermission = true;

    public WiredEffectMatchFurniAndHeight(ResultSet set, Item baseItem) throws SQLException {
        super(set, baseItem);
    }

    public WiredEffectMatchFurniAndHeight(int id, int userId, Item item, String extradata, int limitedStack, int limitedSells) {
    ...
    public boolean saveData(WiredSettings settings, GameClient gameClient) throws WiredSaveException {
        ...
        ArrayList<WiredMatchFurniAndHeightSetting> newSettings = new ArrayList<WiredMatchFurniAndHeightSetting>();
        for (int i = 0; i < itemsCount; ++i) {
            ...
            newSettings.add(new WiredMatchFurniAndHeightSetting(it.getId(), this.checkForWiredResetPermission && it.allowWiredResetState() ? it.getExtradata() : " ", it.getRotation(), it.getX(), it.getY(), it.getZ()));
        }
    ...
    public boolean execute(RoomUnit roomUnit, Room room, Object[] stuff) {
        ...
        for (WiredMatchFurniAndHeightSetting setting : this.settings) {
            RoomTile t;
            ...
            if (this.position && (t = room.getLayout().getTile((short)setting.x, (short)setting.y)) != null && t.state != RoomTileState.INVALID) {
                ...
                if (room.hasHabbosAt(t.x, t.y) && !item.getBaseItem().allowSit()) {
                    canMove = false;
                }
                if (canMove || Emulator.getConfig().getBoolean("hotel.room.wired.norules")) {
                    ...
                    if (highestZ != -1.0) {
                        ...
                        if (!isAddon) {
                            ...
                            item.onMove(room, room.getLayout().getTile(item.getX(), item.getY()), t);
                            item.setX(t.x);
                            item.setY(t.y);
                            item.setZ(item.getZ() + highestZ);
    ...
    public String getWiredData() {
        ...
        if (this.settings.isEmpty()) {
            ...
            Room room = Emulator.getGameEnvironment().getRoomManager().getRoom(this.getRoomId());
            for (WiredMatchFurniAndHeightSetting item : this.settings) {
                HabboItem i;
    ...
    public void loadWiredData(ResultSet set, Room room) throws SQLException {
        ...
        for (String item : items = data[1].split(";")) {
            try {
                ...
                if (stuff.length >= 7) {
                    this.settings.add(new WiredMatchFurniAndHeightSetting(Integer.parseInt(stuff[0]), stuff[1], Integer.parseInt(stuff[2]), Integer.parseInt(stuff[3]), Integer.parseInt(stuff[4]), Double.parseDouble(stuff[5] + "-" + stuff[6])));
                    continue;
                }
                if (stuff.length == 6) {
                    this.settings.add(new WiredMatchFurniAndHeightSetting(Integer.parseInt(stuff[0]), stuff[1], Integer.parseInt(stuff[2]), Integer.parseInt(stuff[3]), Integer.parseInt(stuff[4]), Double.parseDouble(stuff[5])));
                    continue;
                }
                if (stuff.length != 5) continue;
                this.settings.add(new WiredMatchFurniAndHeightSetting(Integer.parseInt(stuff[0]), stuff[1], Integer.parseInt(stuff[2]), Integer.parseInt(stuff[3]), Integer.parseInt(stuff[4]), 0.0));
            }
            catch (Exception e) {
    ...
    private void refresh() {
        ...
        if (room != null && room.isLoaded()) {
            THashSet<WiredMatchFurniAndHeightSetting> remove = new THashSet<WiredMatchFurniAndHeightSetting>();
            for (WiredMatchFurniAndHeightSetting setting : this.settings) {
                HabboItem item = room.getHabboItem(setting.itemId);
                if (item != null) continue;
                remove.add(setting);
            }
            for (WiredMatchFurniAndHeightSetting setting : remove) {
