...
public class WiredTriggerHabboSaysEqualsKeyword
extends InteractionWiredTrigger {
    private static final WiredTriggerType type = WiredTriggerType.SAY_SOMETHING;
    private boolean ownerOnly = false;
    private String key = "";

    public WiredTriggerHabboSaysEqualsKeyword(ResultSet set, Item baseItem) throws SQLException {
        super(set, baseItem);
    }

    public WiredTriggerHabboSaysEqualsKeyword(int id, int userId, Item item, String extradata, int limitedStack, int limitedSells) {
        super(id, userId, item, extradata, limitedStack, limitedSells);
    }

    @Override
    public boolean execute(RoomUnit roomUnit, Room room, Object[] stuff) {
        if (this.key.length() > 0 && stuff[0] instanceof String && ((String)stuff[0]).equalsIgnoreCase(this.key)) {
