...
public class InteractionVendingMachine
extends HabboItem {
    ...
    public THashSet<RoomTile> getActivatorTiles(Room room) {
        ...
        RoomTile tileInFront = InteractionVendingMachine.getSquareInFront(room.getLayout(), this);
        if (tileInFront != null) {
    ...
    private void tryInteract(GameClient client, Room room, RoomUnit unit) {
        ...
        for (RoomTile tile : activatorTiles) {
            if (!unit.getCurrentLocation().is(unit.getX(), unit.getY())) continue;
    ...
    public void onClick(GameClient client, Room room, Object[] objects) throws Exception {
        ...
        boolean inActivatorSpace = false;
        for (RoomTile roomTile : activatorTiles) {
            if (!unit.getCurrentLocation().is(roomTile.x, roomTile.y)) continue;
            inActivatorSpace = true;
        }
        if (!inActivatorSpace) {
            RoomTile tileToWalkTo = null;
            for (RoomTile tile : activatorTiles) {
                if (tile.state != RoomTileState.OPEN && tile.state != RoomTileState.SIT || tileToWalkTo != null && !(tileToWalkTo.distance(unit.getCurrentLocation()) > tile.distance(unit.getCurrentLocation()))) continue;
                tileToWalkTo = tile;
            }
            if (tileToWalkTo != null) {
                ArrayList<Runnable> arrayList = new ArrayList<Runnable>();
                ArrayList<Runnable> onFail = new ArrayList<Runnable>();
                arrayList.add(() -> this.tryInteract(client, room, unit));
                unit.setGoalLocation(tileToWalkTo);
                Emulator.getThreading().run(new RoomUnitWalkToLocation(unit, tileToWalkTo, room, arrayList, onFail));
            }
        } else {
            this.useVendingMachine(client, room, unit);
        }
