...
public class InteractionFreezeTile
extends HabboItem {
    ...
    public void onClick(GameClient client, Room room, Object[] objects) throws Exception {
        ...
        if (client.getHabbo().getRoomUnit().getCurrentLocation().x == this.getX() && client.getHabbo().getRoomUnit().getCurrentLocation().y == this.getY() && (game = (FreezeGame)room.getGame(FreezeGame.class)) != null) {
    ...
    public boolean canStackAt(Room room, List<Pair<RoomTile, THashSet<HabboItem>>> itemsAtLocation) {
        for (Pair<RoomTile, THashSet<HabboItem>> set : itemsAtLocation) {
            if (set.getValue() == null || set.getValue().isEmpty()) continue;
            return false;
        }
