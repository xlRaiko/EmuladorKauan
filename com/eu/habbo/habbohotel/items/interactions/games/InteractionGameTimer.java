...
public class InteractionGameTimer
extends HabboItem
implements Runnable {
    ...
    public InteractionGameTimer(ResultSet set, Item baseItem) throws SQLException {
        ...
        try {
            ...
            if (data.length >= 2) {
                this.timeNow = this.baseTime = Integer.parseInt(data[1]);
            }
            if (data.length >= 1) {
            ...
            this.timeNow = this.baseTime = this.TIMER_INTERVAL_STEPS[0];
        }
    }
    ...
    public void endGame(Room room, boolean isStart) {
        ...
        for (Game game : room.getGames()) {
            if (game.getState().equals((Object)GameState.IDLE) || isStart && game instanceof WiredGame) continue;
            game.onEnd();
            game.stop();
        }
    }
    ...
    public void onClick(GameClient client, Room room, Object[] objects) throws Exception {
        ...
        if (objects.length >= 2 && objects[1] instanceof WiredEffectType) {
            ...
            if (objects.length >= 1 && objects[0] instanceof Integer) {
                state = InteractionGameTimerAction.getByAction((Integer)objects[0]);
            }
    ...
    public static enum InteractionGameTimerAction {
        START_STOP(1),
        INCREASE_TIME(2);
