...
public class InteractionTotemLegs
extends InteractionDefault {
    ...
    public TotemType getTotemType() {
        ...
        return TotemType.fromInt((int)Math.ceil((float)(extraData + 1) / 4.0f));
    }
    ...
    private void updateHead(Room room, RoomTile tile) {
        for (HabboItem item : room.getItemsAt(tile)) {
            if (!(item instanceof InteractionTotemHead) || !(item.getZ() > this.getZ())) continue;
