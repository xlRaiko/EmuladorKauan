...
public class CrackableReward {
        ...
        for (String prize : prizes) {
            try {
                ...
                if (prize.contains(":") && prize.split(":").length == 2) {
                    ...
                    LOGGER.error("Invalid configuration of crackable prizes (item id: {}). '{}' format should be itemId:chance.", (Object)this.itemId, (Object)prize);
                } else {
    ...
    public int getRandomReward() {
        ...
        for (Map.Entry<Integer, Map.Entry<Integer, Integer>> set : this.prizes.entrySet()) {
            notFound = set.getKey();
            if (random < set.getValue().getKey() || random >= set.getValue().getValue()) continue;
            return set.getKey();
        }
        return notFound;
    }
