...
public class RoomTrade {
    ...
    private final List<RoomTradeUser> users = new ArrayList<RoomTradeUser>();
    ...
    protected void initializeTradeStatus() {
        for (RoomTradeUser roomTradeUser : this.users) {
            if (roomTradeUser.getHabbo().getRoomUnit().hasStatus(RoomUnitStatus.TRADING)) continue;
            roomTradeUser.getHabbo().getRoomUnit().setStatus(RoomUnitStatus.TRADING, "");
            if (roomTradeUser.getHabbo().getRoomUnit().isWalking()) continue;
    ...
    public void offerMultipleItems(Habbo habbo, THashSet<HabboItem> items) {
        ...
        for (HabboItem item : items) {
            if (user.getItems().contains(item)) continue;
            habbo.getInventory().getItemsComponent().removeHabboItem(item);
            user.getItems().add(item);
        }
        this.clearAccepted();
    ...
    public void accept(Habbo habbo, boolean value) {
        ...
        for (RoomTradeUser roomTradeUser : this.users) {
            if (roomTradeUser.getAccepted()) continue;
            accepted = false;
        }
        if (accepted) {
    ...
    public void confirm(Habbo habbo) {
        ...
        boolean accepted = true;
        for (RoomTradeUser roomTradeUser : this.users) {
            if (roomTradeUser.getConfirmed()) continue;
            accepted = false;
        }
        if (accepted) {
            if (this.tradeItems()) {
    ...
    boolean tradeItems() {
        for (RoomTradeUser roomTradeUser : this.users) {
            for (HabboItem item : roomTradeUser.getItems()) {
                if (roomTradeUser.getHabbo().getInventory().getItemsComponent().getHabboItem(item.getId()) == null) continue;
        ...
        try (Connection connection = Emulator.getDatabase().getDataSource().getConnection();){
            ...
            if (logTrades) {
                try (PreparedStatement statement = connection.prepareStatement("INSERT INTO room_trade_log (user_one_id, user_two_id, user_one_ip, user_two_ip, timestamp, user_one_item_count, user_two_item_count) VALUES (?, ?, ?, ?, ?, ?, ?)", 1);){
        ...
        THashSet<HabboItem> itemsUserOne = new THashSet<HabboItem>(userOne.getItems());
        THashSet<HabboItem> itemsUserTwo = new THashSet<HabboItem>(userTwo.getItems());
        userOne.clearItems();
        userTwo.clearItems();
        int creditsForUserTwo = 0;
        THashSet<HabboItem> creditFurniUserOne = new THashSet<HabboItem>();
        ...
        THashSet<HabboItem> creditFurniUserTwo = new THashSet<HabboItem>();
    ...
    public RoomTradeUser getRoomTradeUserForHabbo(Habbo habbo) {
        for (RoomTradeUser roomTradeUser : this.users) {
            if (roomTradeUser.getHabbo() != habbo) continue;
