...
implements Disposable {
    ...
    private final List<InteractionMusicDisc> songs = new ArrayList<InteractionMusicDisc>(0);
    ...
    public InteractionJukeBox loadRoomJukebox() {
        try (Connection connection = Emulator.getDatabase().getDataSource().getConnection();
             PreparedStatement statement = connection.prepareStatement("SELECT * FROM room_trax WHERE room_id = ?");){
            ...
            try (ResultSet set = statement.executeQuery();){
                ...
                if (!(jukebox instanceof InteractionJukeBox)) {
                    InteractionJukeBox interactionJukeBox = null;
                    return interactionJukeBox;
                }
                InteractionJukeBox interactionJukeBox = (InteractionJukeBox)jukebox;
                return interactionJukeBox;
            }
        }
        ...
        try (Connection connection = Emulator.getDatabase().getDataSource().getConnection();
            ...
            try (ResultSet set = statement.executeQuery();){
                while (set.next()) {
                    ...
                    if (musicDisc == null) continue;
                    if (!(musicDisc instanceof InteractionMusicDisc) || musicDisc.getRoomId() != -1) {
                        TraxManager.deleteSongFromPlaylist(this.jukeBox.getId(), musicDisc.getId());
                        continue;
                    }
    ...
    public void play(int index, Habbo starter) {
        ...
        if (!this.songs.isEmpty()) {
            this.currentlyPlaying = this.songs.get(index %= this.songs.size());
            if (this.currentlyPlaying != null) {
    ...
    public void removeSong(int itemId) {
        ...
        if (musicDisc != null) {
            ...
            TraxManager.deleteSongFromPlaylist(this.jukeBox.getId(), itemId);
    ...
    public static void removeAllSongs(InteractionJukeBox jukebox) {
        try (Connection connection = Emulator.getDatabase().getDataSource().getConnection();
            ...
            try (ResultSet set = statement.executeQuery();){
                while (set.next()) {
                    HabboItem musicDisc = Emulator.getGameEnvironment().getItemManager().loadHabboItem(set.getInt("item_id"));
                    TraxManager.deleteSongFromPlaylist(jukebox.getId(), set.getInt("item_id"));
                    if (musicDisc == null || !(musicDisc instanceof InteractionMusicDisc) || musicDisc.getRoomId() != -1) continue;
                    musicDisc.setRoomId(0);
                    musicDisc.needsUpdate(true);
                    Emulator.getThreading().run(musicDisc);
                    Habbo owner = Emulator.getGameEnvironment().getHabboManager().getHabbo(musicDisc.getUserId());
    ...
    public List<SoundTrack> soundTrackList() {
        ArrayList<SoundTrack> trax = new ArrayList<SoundTrack>(this.songs.size());
    ...
    public List<InteractionMusicDisc> myList(Habbo habbo) {
        return habbo.getInventory().getItemsComponent().getItems().valueCollection().stream().filter(i -> i instanceof InteractionMusicDisc && i.getRoomId() == 0).map(i -> (InteractionMusicDisc)i).collect(Collectors.toList());
    }

    public InteractionMusicDisc getSong(int itemId) {
        for (InteractionMusicDisc musicDisc : this.songs) {
            if (musicDisc == null || musicDisc.getId() != itemId) continue;
            return musicDisc;
        }
        return null;
    }
    ...
    public void sendUpdatedSongList() {
        this.room.getHabbos().forEach(h -> {
            ...
            if (client != null) {
                client.sendResponse(new JukeBoxMySongsComposer(this.myList((Habbo)h)));
            }
        });
    }
