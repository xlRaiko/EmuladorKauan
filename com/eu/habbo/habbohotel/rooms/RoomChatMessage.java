...
DatabaseLoggable {
    ...
    public RoomChatMessage(MessageHandler message) {
        ...
        if (!message.client.getHabbo().hasPermission(Permission.ACC_ANYCHATCOLOR)) {
            int[] nArray = BANNED_BUBBLES;
            int n = nArray.length;
            for (int i = 0; i < n; ++i) {
                Integer i2 = nArray[i];
                if (i2.intValue() != this.bubble.getType()) continue;
                this.bubble = RoomChatMessageBubbles.NORMAL;
                break;
            }
        }
        ...
        if (Emulator.getConfig().getBoolean("hotel.wordfilter.enabled") && Emulator.getConfig().getBoolean("hotel.wordfilter.rooms") && !this.habbo.hasPermission(Permission.ACC_CHAT_NO_FILTER)) {
            if (!Emulator.getGameEnvironment().getWordFilter().autoReportCheck(this)) {
                ...
                if (muteTime > 0) {
                    ...
                    LOGGER.error("Invalid hotel.wordfilter.automute defined in emulator_settings ({}).", (Object)muteTime);
                }
            }
            this.message = "";
        }
