...
public class RoomUnit {
    ...
    public void stopWalking() {
        ConcurrentHashMap<RoomUnitStatus, String> concurrentHashMap = this.status;
        synchronized (concurrentHashMap) {
            this.status.remove((Object)RoomUnitStatus.MOVE);
    ...
    public boolean cycle(Room room) {
        try {
            RoomUnit ridingUnit;
            Optional<HabboItem> stackHelper;
            HabboItem habboItem;
            ...
            Pet pet;
            Habbo rider = null;
            if (this.getRoomUnitType() == RoomUnitType.PET && (pet = room.getPet(this)) instanceof RideablePet) {
                rider = ((RideablePet)pet).getRider();
            }
            if (rider != null) {
                if (this.status.containsKey((Object)RoomUnitStatus.MOVE) && !rider.getRoomUnit().getStatusMap().containsKey((Object)RoomUnitStatus.MOVE)) {
                    this.status.remove((Object)RoomUnitStatus.MOVE);
                }
            ...
            if (!this.isWalking() && !this.isKicked && this.status.remove((Object)RoomUnitStatus.MOVE) == null) {
                Habbo habboT = room.getHabbo(this);
                if (habboT != null) {
                    habboT.getHabboInfo().getRiding().getRoomUnit().status.remove((Object)RoomUnitStatus.MOVE);
                }
                return true;
            }
            if (this.status.remove((Object)RoomUnitStatus.SIT) != null) {
                this.statusUpdate = true;
            }
            if (this.status.remove((Object)RoomUnitStatus.MOVE) != null) {
                this.statusUpdate = true;
            }
            if (this.status.remove((Object)RoomUnitStatus.LAY) != null) {
                this.statusUpdate = true;
            }
            for (Map.Entry entry : this.status.entrySet()) {
                if (!((RoomUnitStatus)((Object)entry.getKey())).removeWhenWalking) continue;
            ...
            boolean bl = overrideChecks = (next = this.path.poll()) != null && this.canOverrideTile(next);
            ...
            this.status.remove((Object)RoomUnitStatus.DEAD);
            if (habbo2 != null) {
                if (this.isIdle()) {
                    UserIdleEvent event = new UserIdleEvent(habbo2, UserIdleEvent.IdleReason.WALKED, false);
                ...
                if (Emulator.getPluginManager().isRegistered(UserTakeStepEvent.class, false)) {
                    UserTakeStepEvent e = new UserTakeStepEvent(habbo2, room.getLayout().getTile(this.getX(), this.getY()), next);
            ...
            if (!(this.path.isEmpty() && canSitNextTile || !RoomUnit.canMoveToTile(room, next, height = next.getStackHeight() - this.currentLocation.getStackHeight(), canSitNextTile))) {
                this.path.clear();
                this.status.remove((Object)RoomUnitStatus.MOVE);
                return false;
            }
            if (canSitNextTile && (tallestChair = room.getTallestChair(next)) != null) {
                item = tallestChair;
            }
            if (next.equals(this.goalLocation) && next.state == RoomTileState.SIT && !overrideChecks && (item == null || item.getZ() - this.getZ() > RoomLayout.MAXIMUM_STEP_HEIGHT)) {
                this.status.remove((Object)RoomUnitStatus.MOVE);
                return false;
            }
            double zHeight = 0.0;
            ...
            if (!((habboItem = room.getTopItemAt(this.getX(), this.getY())) == null || habboItem == item && RoomLayout.pointInSquare(habboItem.getX(), habboItem.getY(), habboItem.getX() + habboItem.getBaseItem().getWidth() - 1, habboItem.getY() + habboItem.getBaseItem().getLength() - 1, next.x, next.y))) {
            ...
            if (item != null) {
                if (item != habboItem || !RoomLayout.pointInSquare(item.getX(), item.getY(), item.getX() + item.getBaseItem().getWidth() - 1, item.getY() + item.getBaseItem().getLength() - 1, this.getX(), this.getY())) {
                    if (item.canWalkOn(this, room, null)) {
                        ...
                        if (habbo2 != null) {
                            ((ConditionalGate)((Object)item)).onRejected(this, this.getRoom(), new Object[0]);
                        }
                        return false;
                    }
                } else {
                ...
                zHeight += (double)room.getLayout().getHeightAtSquare(next.x, next.y);
            }
            if ((stackHelper = room.getItemsAt(next).stream().filter(i -> i instanceof InteractionTileWalkMagic).findAny()).isPresent()) {
            ...
            if (habbo2 != null && habbo2.getHabboInfo().getRiding() != null && (ridingUnit = habbo2.getHabboInfo().getRiding().getRoomUnit()) != null) {
            ...
            if (habbo2 != null) {
                ...
                boolean bl2 = invalidated = topItem != null && topItem.invalidatesToRoomKick();
                if (this.canLeaveRoomByDoor && isAtDoor && publicRoomKicks && !invalidated) {
                    Emulator.getThreading().run(new RoomUnitKick(habbo2, room, false), 500L);
                }
            }
            return false;
        }
    ...
    private static boolean canMoveToTile(Room room, RoomTile next, double height, boolean canSitNextTile) {
        return (!room.tileWalkable(next) || !RoomLayout.ALLOW_FALLING && height < -RoomLayout.MAXIMUM_STEP_HEIGHT || next.state == RoomTileState.OPEN && height > RoomLayout.MAXIMUM_STEP_HEIGHT) && !canSitNextTile;
    }
    ...
    public void setGoalLocation(RoomTile goalLocation, boolean noReset) {
        if (Emulator.getPluginManager().isRegistered(RoomUnitSetGoalEvent.class, false)) {
            RoomUnitSetGoalEvent event = new RoomUnitSetGoalEvent(this.room, this, goalLocation);
            Emulator.getPluginManager().fireEvent(event);
    ...
    public void findPath() {
        ...
        Deque<RoomTile> newPath = this.room.getLayout().getPathfinder().findPath(this.currentLocation, this.goalLocation, this.goalLocation, this);
    ...
    private boolean canFindPath() {
        return this.room != null && this.room.getLayout() != null && this.goalLocation != null && (this.goalLocation.isWalkable() || this.room.canSitOrLayAt(this.goalLocation.x, this.goalLocation.y) || this.canOverrideTile(this.goalLocation));
    }
    ...
    public String getStatus(RoomUnitStatus key) {
        return this.status.get((Object)key);
    }
    ...
    public boolean hasStatus(RoomUnitStatus key) {
        return this.status.containsKey((Object)key);
    }
    ...
    public void lookAtPoint(RoomTile location) {
        ...
        if (Emulator.getPluginManager().isRegistered(RoomUnitLookAtPointEvent.class, false)) {
            RoomUnitLookAtPointEvent lookAtPointEvent = new RoomUnitLookAtPointEvent(this.room, this, location);
            Emulator.getPluginManager().fireEvent(lookAtPointEvent);
            if (lookAtPointEvent.isCancelled()) {
                return;
            }
        }
        if (this.status.containsKey((Object)RoomUnitStatus.LAY)) {
            return;
        }
        if (!this.status.containsKey((Object)RoomUnitStatus.SIT)) {
            this.bodyRotation = RoomUserRotation.values()[Rotation.Calculate(this.getX(), this.getY(), location.x, location.y)];
        }
        if (Math.abs((rotation = RoomUserRotation.values()[Rotation.Calculate(this.getX(), this.getY(), location.x, location.y)]).getValue() - this.bodyRotation.getValue()) <= 1) {
            this.headRotation = rotation;
        }
    }
    ...
    public RoomTile getClosestAdjacentTile(short x, short y, boolean diagonal) {
        ...
        return this.getClosestTile(rotations.stream().map(rotation -> this.room.getLayout().getTileInFront(baseTile, (int)rotation)).filter(t -> t != null && t.isWalkable() && (this.getCurrentLocation().equals(t) || !this.room.hasHabbosAt(t.x, t.y))).collect(Collectors.toList()));
    }
