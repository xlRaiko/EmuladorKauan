...
extends Game {
    ...
    public static void onUserLookAtPoint(RoomUnitLookAtPointEvent event) {
        ...
        if (RoomLayout.tilesAdjecent(event.roomUnit.getCurrentLocation(), event.location) && (habbo = event.room.getHabbo(event.roomUnit)) != null && habbo.getHabboInfo().getCurrentGame() != null && TagGame.class.isAssignableFrom(habbo.getHabboInfo().getCurrentGame()) && (game = (TagGame)event.room.getGame(habbo.getHabboInfo().getCurrentGame())) != null && game.isTagger(habbo)) {
            for (Habbo tagged : event.room.getHabbosAt(event.location)) {
                if (tagged == habbo || tagged.getHabboInfo().getCurrentGame() == null || tagged.getHabboInfo().getCurrentGame() != habbo.getHabboInfo().getCurrentGame()) continue;
        ...
        if (event.habbo.getHabboInfo().getCurrentGame() != null && TagGame.class.isAssignableFrom(event.habbo.getHabboInfo().getCurrentGame())) {
            ...
            if (game != null) {
                for (HabboItem item : items) {
                    if (!(item instanceof InteractionTagField) || ((InteractionTagField)item).gameClazz != event.habbo.getHabboInfo().getCurrentGame()) continue;
    ...
    public synchronized boolean addHabbo(Habbo habbo, GameTeamColors teamColor) {
        ...
        if (this.getTagPole() != null) {
            ...
            if (poles.size() > this.taggers.size()) {
                ...
                Iterator iterator = poles.iterator();
                if (((THashIterator)iterator).hasNext()) {
                    HabboItem item = (HabboItem)((THashIterator)iterator).next();
    ...
    public int getEffect(Habbo habbo) {
        if (habbo.getHabboInfo().getGender().equals((Object)HabboGender.M)) {
