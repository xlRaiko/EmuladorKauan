    ...
    public TalentTrackLevel(ResultSet set) throws SQLException {
        ...
        if (achievementLevels.length == achievements.length) {
            for (int i = 0; i < achievements.length; ++i) {
                ...
                LOGGER.error("Could not find achievement with ID {} for talenttrack level {} of type {}", new Object[]{achievements[i], this.level, this.type});
            }
        }
        for (String s : set.getString("reward_furni").split(",")) {
            ...
            LOGGER.error("Incorrect reward furni (ID: {}) for talent track level {}", (Object)s, (Object)this.level);
        }
