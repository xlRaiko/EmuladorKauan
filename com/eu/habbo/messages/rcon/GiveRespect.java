...
public class GiveRespect
extends RCONMessage<JSONGiveRespect> {
    ...
    public void handle(Gson gson, JSONGiveRespect object) {
        ...
        if (habbo != null) {
            ...
            try (Connection connection = Emulator.getDatabase().getDataSource().getConnection();
                 PreparedStatement statement = connection.prepareStatement("UPDATE users_settings SET respects_given = respects_given + ?, respects_received = respects_received + ?, daily_respect_points = daily_respect_points + ? WHERE user_id = ? LIMIT 1");){
