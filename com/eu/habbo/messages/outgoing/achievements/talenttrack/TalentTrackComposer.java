...
extends MessageComposer {
        ...
        if (talentTrackLevels != null) {
            ...
            for (Map.Entry<Integer, TalentTrackLevel> set : talentTrackLevels.entrySet()) {
                try {
                    ...
                    TalentTrackState finalState = state;
                    level.achievements.forEachEntry((achievement, index) -> {
                        if (achievement != null) {
                            ...
                            int progress = Math.max(0, this.habbo.getHabboStats().getAchievementProgress((Achievement)achievement));
    ...
    public static enum TalentTrackState {
        LOCKED(0),
        IN_PROGRESS(1),
        COMPLETED(2);
        ...
        private TalentTrackState(int id) {
            this.id = id;
        }
    }
}
