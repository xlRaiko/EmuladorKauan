...
public class GuildInfoComposer
extends MessageComposer {
    ...
    protected ServerMessage composeInternal() {
        boolean adminPermissions = this.client.getHabbo().getHabboStats().hasGuild(this.guild.getId()) && this.client.getHabbo().hasPermission(Permission.ACC_GUILD_ADMIN) || Emulator.getGameEnvironment().getGuildManager().getOnlyAdmins(this.guild).get(this.client.getHabbo().getHabboInfo().getId()) != null;
        ...
        this.response.appendString(new SimpleDateFormat("dd-MM-yyyy").format(new Date((long)this.guild.getDateCreated() * 1000L)));
        this.response.appendBoolean(adminPermissions || this.guild.getOwnerId() == this.client.getHabbo().getHabboInfo().getId());
        this.response.appendBoolean(adminPermissions || this.member != null && this.member.getRank().equals((Object)GuildRank.ADMIN));
        ...
        this.response.appendInt(adminPermissions || this.guild.getOwnerId() == this.client.getHabbo().getHabboInfo().getId() ? this.guild.getRequestCount() : 0);
