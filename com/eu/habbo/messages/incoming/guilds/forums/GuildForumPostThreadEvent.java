...
public class GuildForumPostThreadEvent
extends MessageHandler {
    ...
    public void handle() throws Exception {
        ...
        if (threadId == 0) {
            if (!(guild.canPostThreads().state == 0 || guild.canPostThreads().state == 1 && member != null || guild.canPostThreads().state == 2 && member != null && member.getRank().type < GuildRank.MEMBER.type || guild.canPostThreads().state == 3 && guild.getOwnerId() == this.client.getHabbo().getHabboInfo().getId() || isStaff)) {
        ...
        if (!(guild.canPostMessages().state == 0 || guild.canPostMessages().state == 1 && member != null || guild.canPostMessages().state == 2 && member != null && member.getRank().type < GuildRank.MEMBER.type || guild.canPostMessages().state == 3 && guild.getOwnerId() == this.client.getHabbo().getHabboInfo().getId() || isStaff)) {
            this.client.sendResponse(new ConnectionErrorComposer(403));
            return;
        }
