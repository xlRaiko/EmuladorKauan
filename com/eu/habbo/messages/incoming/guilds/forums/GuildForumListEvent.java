...
extends MessageHandler {
    ...
    private THashSet<Guild> getActiveForums() {
        ...
        try (Connection connection = Emulator.getDatabase().getDataSource().getConnection();
             PreparedStatement statement = connection.prepareStatement("SELECT `guilds`.`id`, SUM(`guilds_forums_threads`.`posts_count`) AS `post_count` FROM `guilds_forums_threads` LEFT JOIN `guilds` ON `guilds`.`id` = `guilds_forums_threads`.`guild_id` WHERE `guilds`.`read_forum` = 'EVERYONE' AND `guilds_forums_threads`.`created_at` > ? GROUP BY `guilds`.`id` ORDER BY `post_count` DESC LIMIT 100");){
    ...
    private THashSet<Guild> getMyForums(int userId) {
        THashSet<Guild> guilds = new THashSet<Guild>();
        try (Connection connection = Emulator.getDatabase().getDataSource().getConnection();
             PreparedStatement statement = connection.prepareStatement("SELECT `guilds`.`id` FROM `guilds_members` LEFT JOIN `guilds` ON `guilds`.`id` = `guilds_members`.`guild_id` WHERE `guilds_members`.`user_id` = ? AND `guilds`.`forum` = '1'");){
