...
public class SetStackHelperHeightEvent
extends MessageHandler {
    ...
    public void handle() throws Exception {
        ...
        if ((this.client.getHabbo().getHabboInfo().getId() == this.client.getHabbo().getHabboInfo().getCurrentRoom().getOwnerId() || this.client.getHabbo().getHabboInfo().getCurrentRoom().hasRights(this.client.getHabbo())) && ((item = this.client.getHabbo().getHabboInfo().getCurrentRoom().getHabboItem(itemId)) instanceof InteractionStackHelper || item instanceof InteractionTileWalkMagic)) {
            ...
            if (stackerHeight == -100.0) {
                for (RoomTile tile : tiles) {
                    ...
                    if (!(stackheight > stackerHeight)) continue;
                    stackerHeight = stackheight;
                }
            } else {
                stackerHeight = Math.min(Math.max(stackerHeight, (double)(itemTile.z * 100)), Room.MAXIMUM_FURNI_HEIGHT * 100.0);
            }
            double height = 0.0;
            ...
            this.client.getHabbo().getHabboInfo().getCurrentRoom().sendComposer(new UpdateStackHeightTileHeightComposer(item, (int)(height * 100.0)).compose());
        }
    }
}
