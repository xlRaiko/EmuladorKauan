    ...
    public void handle() throws Exception {
        ...
        if (item != null) {
            if (item instanceof InteractionDice) {
                if (RoomLayout.tilesAdjecent(room.getLayout().getTile(item.getX(), item.getY()), this.client.getHabbo().getRoomUnit().getCurrentLocation()) && !item.getExtradata().equals("-1")) {
                    item.setExtradata("0");
