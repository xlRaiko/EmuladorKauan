...
public class PostItSaveDataEvent
extends MessageHandler {
    ...
    public void handle() throws Exception {
        ...
        String text = Emulator.getGameEnvironment().getWordFilter().filter(this.packet.readString().replace("\t", ""), this.client.getHabbo());
        ...
        if (!color.equalsIgnoreCase(PostItColor.YELLOW.hexColor) && !room.hasRights(this.client.getHabbo()) ? !text.startsWith(item.getExtradata().replace(item.getExtradata().split(" ")[0], "")) : !room.hasRights(this.client.getHabbo())) {
            return;
        }
        if (color.isEmpty()) {
