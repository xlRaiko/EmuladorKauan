...
public class CatalogSelectClubGiftEvent
extends MessageHandler {
    ...
    public void handle() throws Exception {
        ...
        if (daysRequired > (int)Math.floor((double)this.client.getHabbo().getHabboStats().getPastTimeAsClub() / 86400.0)) {
            LOGGER.error("Not been member for long enough");
            this.client.sendResponse(new AlertPurchaseFailedComposer(0));
            return;
        }
        THashSet<Item> itemsGiven = new THashSet<Item>();
        for (Item item : catalogItem.getBaseItems()) {
            if (Emulator.getGameEnvironment().getItemManager().createGift(this.client.getHabbo().getHabboInfo().getId(), item, "", 0, 0) == null) continue;
            itemsGiven.add(item);
        }
