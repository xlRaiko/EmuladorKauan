...
public class CatalogSearchedItemEvent
extends MessageHandler {
    ...
    public void handle() throws Exception {
        ...
        if (pageId != 0 && (page = Emulator.getGameEnvironment().getCatalogManager().getCatalogPage(Emulator.getGameEnvironment().getCatalogManager().getCatalogItem(pageId).getPageId())) != null) {
            TIntObjectIterator<CatalogItem> iterator = page.getCatalogItems().iterator();
            while (iterator.hasNext()) {
                iterator.advance();
                CatalogItem item = iterator.value();
                if (item.getOfferId() != offerId) continue;
