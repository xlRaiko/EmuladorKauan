...
public class RequestCreateRoomEvent
extends MessageHandler {
    ...
    public void handle() throws Exception {
        ...
        if (!Emulator.getGameEnvironment().getRoomManager().layoutExists(modelName)) {
            LOGGER.error("[SCRIPTER] Incorrect layout name \"{}\". {}", (Object)modelName, (Object)this.client.getHabbo().getHabboInfo().getUsername());
            return;
        }
        ...
        if (category == null || category.getMinRank() > this.client.getHabbo().getHabboInfo().getRank().getId()) {
            LOGGER.error("[SCRIPTER] Incorrect rank or non existing category ID: \"{}\".{}", (Object)categoryId, (Object)this.client.getHabbo().getHabboInfo().getUsername());
            return;
        }
        if (maxUsers > 250) {
            return;
        ...
        int n = max = this.client.getHabbo().getHabboStats().hasActiveClub() ? RoomManager.MAXIMUM_ROOMS_HC : RoomManager.MAXIMUM_ROOMS_USER;
        if (count >= max) {
            this.client.sendResponse(new CanCreateRoomComposer(count, max));
            return;
        }
        Room room = Emulator.getGameEnvironment().getRoomManager().createRoomForHabbo(this.client.getHabbo(), name, description, modelName, maxUsers, categoryId, tradeType);
        if (room != null) {
