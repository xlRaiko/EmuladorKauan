...
import com.eu.habbo.messages.incoming.handshake.IsFirstLoginOfDayComposer;
...
extends MessageHandler {
    ...
    public void handle() throws Exception {
        ...
        if (this.client.getHabbo() == null) {
            ...
            if (habbo != null) {
                THashMap<Integer, ArrayList<ModToolSanctionItem>> modToolSanctionItemsHashMap;
                ...
                if (ClothingValidationManager.VALIDATE_ON_LOGIN && !(validated = ClothingValidationManager.validateLook(this.client.getHabbo())).equals(this.client.getHabbo().getHabboInfo().getLook())) {
                ...
                if (Emulator.getConfig().getBoolean("hotel.sanctions.enabled") && (modToolSanctionItems = (modToolSanctionItemsHashMap = Emulator.getGameEnvironment().getModToolSanctions().getSanctions(habbo.getHabboInfo().getId())).get(habbo.getHabboInfo().getId())) != null && modToolSanctionItems.size() > 0) {
                    ...
                    if (item.isMuted && item.muteDuration <= Emulator.getIntUnixTimestamp()) {
                        ...
                        Date muteDuration = new Date((long)item.muteDuration * 1000L);
                ...
                if (Emulator.getConfig().getBoolean("hotel.welcome.alert.enabled")) {
                    ...
                    Emulator.getThreading().run(() -> {
                        if (Emulator.getConfig().getBoolean("hotel.welcome.alert.oldstyle")) {
                            this.client.sendResponse(new MessagesForYouComposer(HabboManager.WELCOME_MESSAGE.replace("%username%", finalHabbo.getHabboInfo().getUsername()).replace("%user%", finalHabbo.getHabboInfo().getUsername()).split("<br/>")));
                        } else {
                            this.client.sendResponse(new GenericAlertComposer(HabboManager.WELCOME_MESSAGE.replace("%username%", finalHabbo.getHabboInfo().getUsername()).replace("%user%", finalHabbo.getHabboInfo().getUsername())));
                        }
