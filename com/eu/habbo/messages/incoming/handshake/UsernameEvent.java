...
extends MessageHandler {
    ...
    public void handle() throws Exception {
        ...
        if (!this.client.getHabbo().getHabboStats().getAchievementProgress().containsKey(Emulator.getGameEnvironment().getAchievementManager().getAchievement("Login"))) {
            ...
            long daysBetween2 = ChronoUnit.DAYS.between(new Date((long)this.client.getHabbo().getHabboInfo().getLastOnline() * 1000L).toInstant(), new Date().toInstant());
            ...
            if (daysBetween2 == 1L) {
                ...
                calendar = true;
            } else if (daysBetween2 >= 1L) {
                calendar = true;
            } else if (lastLogin.getTime() / 1000L - (long)Emulator.getIntUnixTimestamp() > 86400L) {
        ...
        if (!this.client.getHabbo().getHabboStats().getAchievementProgress().containsKey(Emulator.getGameEnvironment().getAchievementManager().getAchievement("RegistrationDuration"))) {
            ...
            if (daysRegistered - (days = this.client.getHabbo().getHabboStats().getAchievementProgress(Emulator.getGameEnvironment().getAchievementManager().getAchievement("RegistrationDuration"))) > 0) {
        ...
        if (Emulator.getConfig().getBoolean("hotel.calendar.enabled") && (campaign = Emulator.getGameEnvironment().getCalendarManager().getCalendarCampaign(Emulator.getConfig().getValue("hotel.calendar.default"))) != null && (daysBetween = ChronoUnit.DAYS.between(new Timestamp((long)campaign.getStartTimestamp().intValue() * 1000L).toInstant(), new Date().toInstant())) >= 0L) {
        ...
        if (TargetOffer.ACTIVE_TARGET_OFFER_ID > 0 && (offer = Emulator.getGameEnvironment().getCatalogManager().getTargetOffer(TargetOffer.ACTIVE_TARGET_OFFER_ID)) != null) {
