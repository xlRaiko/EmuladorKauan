...
public class WiredTriggerSaveDataEvent
extends MessageHandler {
    ...
    public void handle() throws Exception {
        ...
        if (room != null && (room.hasRights(this.client.getHabbo()) || room.getOwnerId() == this.client.getHabbo().getHabboInfo().getId() || this.client.getHabbo().hasPermission(Permission.ACC_ANYROOMOWNER) || this.client.getHabbo().hasPermission(Permission.ACC_MOVEROTATE)) && (trigger = room.getRoomSpecialTypes().getTrigger(itemId)) != null) {
            ...
            if (saveMethod.isPresent()) {
                if (saveMethod.get().getParameterTypes()[0] == WiredSettings.class) {
                    WiredSettings settings = trigger.readSettings(this.packet, false);
                ...
                } else if (((Boolean)saveMethod.get().invoke((Object)trigger, this.packet)).booleanValue()) {
                    this.client.sendResponse(new WiredSavedComposer());
                    trigger.needsUpdate(true);
                    Emulator.getThreading().run(trigger);
                } else {
