...
public class ReportFriendPrivateChatEvent
extends MessageHandler {
    ...
    public void handle() throws Exception {
        ...
        HabboInfo info = target != null ? target.getHabboInfo() : HabboManager.getOfflineHabboInfo(userId);
        if (info != null) {
            for (int i = 0; i < count; ++i) {
                int chatUserId = this.packet.readInt();
                String username = this.packet.readInt().intValue() == info.getId() ? info.getUsername() : this.client.getHabbo().getHabboInfo().getUsername();
