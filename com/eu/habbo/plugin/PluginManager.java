...
public class PluginManager {
    ...
    public static void globalOnConfigurationUpdated(EmulatorConfigUpdatedEvent event) {
        ...
        try {
            SubscriptionHabboClub.HC_PAYDAY_NEXT_DATE = (int)(Emulator.stringToDate(Emulator.getConfig().getValue("subscriptions.hc.payday.next_date")).getTime() / 1000L);
        }
        catch (Exception e) {
        ...
        SubscriptionHabboClub.HC_PAYDAY_KICKBACK_PERCENTAGE = (double)Emulator.getConfig().getInt("subscriptions.hc.payday.percentage", 10) / 100.0;
    ...
    public void loadPlugins() {
        ...
        for (File file2 : Objects.requireNonNull(loc.listFiles(file -> file.getPath().toLowerCase().endsWith(".jar")))) {
            try {
                URLClassLoader urlClassLoader = URLClassLoader.newInstance(new URL[]{file2.toURI().toURL()});
                ...
                if (stream.read(content) <= 0) continue;
                ...
                try {
                    ...
                    Class<HabboPlugin> stackClazz = clazz.asSubclass(HabboPlugin.class);
                    Constructor<HabboPlugin> constructor = stackClazz.getConstructor(new Class[0]);
                    HabboPlugin plugin = constructor.newInstance(new Object[0]);
                    ...
                    LOGGER.error("Could not load plugin {}!", (Object)pluginConfigurtion.name);
                    LOGGER.error("Caught exception", e);
                }
            }
            catch (Exception e) {
    ...
    public void registerEvents(HabboPlugin plugin, EventListener listener) {
        THashMap<Class<? extends Event>, THashSet<Method>> tHashMap = plugin.registeredEvents;
        synchronized (tHashMap) {
            Method[] methods;
            for (Method method : methods = listener.getClass().getMethods()) {
                if (method.getAnnotation(EventHandler.class) == null || method.getParameterTypes().length != 1 || !Event.class.isAssignableFrom(method.getParameterTypes()[0])) continue;
    ...
    public <T extends Event> T fireEvent(T event) {
        ...
        while (((THashIterator)iterator).hasNext()) {
            try {
                THashSet<Method> methods;
                HabboPlugin plugin = (HabboPlugin)((THashIterator)iterator).next();
                if (plugin == null || (methods = plugin.registeredEvents.get(event.getClass().asSubclass(Event.class))) == null) continue;
                for (Method method : methods) {
                    try {
                        method.invoke((Object)plugin, event);
                    }
                    catch (Exception e) {
                        LOGGER.error("Could not pass event {} to {}", (Object)event.getClass().getName(), (Object)plugin.configuration.name);
                        LOGGER.error("Caught exception", e);
                    }
                }
            }
    ...
    public boolean isRegistered(Class<? extends Event> clazz, boolean pluginsOnly) {
        ...
        while (((THashIterator)iterator).hasNext()) {
            try {
                HabboPlugin plugin = (HabboPlugin)((THashIterator)iterator).next();
                if (!plugin.isRegistered(clazz)) continue;
                return true;
            }
            catch (NoSuchElementException e) {
                // empty catch block
        ...
        if (!pluginsOnly) {
            for (Method method : this.methods) {
                if (method.getParameterTypes().length != 1 || !method.getParameterTypes()[0].isAssignableFrom(clazz)) continue;
                return true;
            }
        }
        return false;
    ...
    private void disposePlugins() {
        ...
        while (((THashIterator)iterator).hasNext()) {
            try {
                HabboPlugin p = (HabboPlugin)((THashIterator)iterator).next();
                if (p == null) continue;
                try {
                    p.onDisable();
                    p.stream.close();
                    ...
                    LOGGER.error("Failed to disable {} because of an exception.", (Object)p.configuration.name, (Object)ex);
                }
            }
            catch (NoSuchElementException e) {
                // empty catch block
    ...
    public void reload() {
        ...
        this.methods.clear();
        this.loadPlugins();
        LOGGER.info("Plugin Manager -> Loaded! {} plugins! ({} MS)", (Object)this.plugins.size(), (Object)(System.currentTimeMillis() - millis));
