...
implements DatabaseUrlContainer {
    ...
    private List<HostInfo> srvRecordsToHostsList(List<DnsSrv.SrvRecord> srvRecords, HostInfo baseHostInfo) {
        return srvRecords.stream().map(s -> this.buildHostInfo(s.getTarget(), s.getPort(), baseHostInfo.getUser(), baseHostInfo.getPassword(), baseHostInfo.isPasswordless(), baseHostInfo.getHostProperties())).collect(Collectors.toList());
    }
