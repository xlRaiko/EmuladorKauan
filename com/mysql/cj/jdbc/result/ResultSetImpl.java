...
WarningListener {
    ...
    public Time getTime(int columnIndex, Calendar cal) throws SQLException {
        try {
            this.checkRowPos();
            this.checkColumnBounds(columnIndex);
            SqlTimeValueFactory vf = new SqlTimeValueFactory(this.session.getPropertySet(), cal, cal != null ? cal.getTimeZone() : this.session.getServerSession().getServerTimeZone());
            return this.thisRow.getValue(columnIndex - 1, vf);
        }
        catch (CJException cJException) {
            throw SQLExceptionsMapping.translateException(cJException, this.getExceptionInterceptor());
    ...
    public Timestamp getTimestamp(int columnIndex, Calendar cal) throws SQLException {
        try {
            ...
            this.checkRowPos();
            this.checkColumnBounds(columnIndex);
            TimeZone timeZone = tz = cal != null ? cal.getTimeZone() : this.session.getServerSession().getServerTimeZone();
            if (this.customTsVf != null && tz == this.lastTsCustomTz) {
                return this.thisRow.getValue(columnIndex - 1, this.customTsVf);
            }
            SqlTimestampValueFactory vf = new SqlTimestampValueFactory(this.session.getPropertySet(), cal, tz);
