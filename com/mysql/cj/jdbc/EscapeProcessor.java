...
class EscapeProcessor {
    ...
    public static final Object escapeSQL(String sql, TimeZone serverTimeZone, boolean serverSupportsFractionalSecond, boolean serverTruncatesFractionalSecond, ExceptionInterceptor exceptionInterceptor) throws SQLException {
        int nextEndBrace;
        ...
        block4: while (escapeTokenizer.hasMoreTokens()) {
            ...
            if (token.charAt(0) == '{') {
                ...
                if (token.length() > 2 && (nestedBrace = token.indexOf(123, 2)) != -1) {
                    ...
                    Object remainingResults = EscapeProcessor.escapeSQL(token.substring(1, token.length() - 1), serverTimeZone, serverSupportsFractionalSecond, serverTruncatesFractionalSecond, exceptionInterceptor);
                ...
                if (StringUtils.startsWithIgnoreCase(collapsedToken, "{ts")) {
                    EscapeProcessor.processTimestampToken(serverTimeZone, newSql, token, serverSupportsFractionalSecond, serverTruncatesFractionalSecond, exceptionInterceptor);
                    continue;
                }
